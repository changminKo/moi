...
export interface OcoGroupState {
  readonly legs: readonly [string, string];
  status: 'ACTIVE' | 'RESOLVED';
  winnerLegId?: string;
}
export interface OcoTriggerContext extends Partial<PricingContext> {
  readonly bothConditionsTrue?: boolean;
  readonly stopLegId?: string;
  readonly takeProfitLegId?: string;
}
export interface OcoExecutionInput {
  readonly groupId: string;
  readonly legId: string;
  readonly siblingId: string;
  readonly pricingContext: OcoTriggerContext;
}
...
export class OcoExecutor {
  ...
  constructor(options: OcoExecutorOptions) {
    this.#options = options;
  }
  group(groupId: string): OcoGroupState | undefined {
    return this.#options.groups.get(groupId);
  }

  async trigger(
    groupId: string,
    legId: string,
    pricingContext: OcoTriggerContext,
  ): Promise<boolean> {
    ...
    try {
      if (
        pricingContext.source === 'RECOVERY_REST' &&
        this.#options.isMarketOpen?.(pricingContext) === false
      )
        return false;
      ...
      if (legId !== first && legId !== second)
        throw new DomainError(
          'ORDER_STATE_CONFLICT',
          'Leg is not in OCO group',
        );
      let winner = legId;
      if (
        pricingContext.source === 'RECOVERY_REST' &&
        pricingContext.bothConditionsTrue === true
      ) {
        winner = chooseRecoveryLeg(
          pricingContext.stopLegId ?? first,
          pricingContext.takeProfitLegId ?? second,
          true,
          legId,
        );
      }
      ...
      release();
      if (this.#chains.get(groupId) === current) this.#chains.delete(groupId);
    }
  }
}
