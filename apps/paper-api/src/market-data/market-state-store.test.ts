...
describe('MarketStateStore', () => {
  it('rejects stale epochs and tokens without advancing the version', () => {
    ...
    expect(
      store.applyEvent({ symbol: 'AAPL', version: 1n, payload: 'one' }),
    ).toMatchObject({ marketDataVersion: 1n });
    expect(() =>
      store.applyEvent({
        symbol: 'AAPL',
        version: 2n,
  ...
  it('rejects out-of-order symbol versions and accepts baselines', () => {
    ...
    expect(() =>
      store.applyEvent({ symbol: 'AAPL', version: 4n, payload: 'again' }),
    ).toThrow(/monotonic|order/i);
    expect(store.replaceBaseline('AAPL', 'baseline')).toMatchObject({
