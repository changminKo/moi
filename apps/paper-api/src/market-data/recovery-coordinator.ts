...
export interface SubscriptionDeclaration {
  readonly channel: 'trade' | 'orderBook';
...
export interface MarketDataStream {
  ...
  declare(subscriptions: readonly SubscriptionDeclaration[]): Promise<{
    accepted: readonly string[];
    rejected: readonly { topic: string; reason: string }[];
  }>;
}
...
export interface RecoveryIncidentPort {
  activate(input: {
    market: Market;
    symbol?: string;
    causeCode: string;
...
export interface RecoveryCoordinatorOptions {
  ...
  readonly evaluateRecovery?: (
    snapshot: RecoverySnapshot,
  ) => readonly Omit<RecoveryTrigger, 'book'>[];
}
...
export class RecoveryCoordinator {
  ...
  async recover(market: Market, signal: AbortSignal): Promise<RecoveryOutcome> {
    ...
    this.#o.stateStore.beginEpoch({
      recoveryEpoch: epoch,
      leaderFencingToken: lease.fencingToken,
    });
    await this.#o.stream.connect(signal);
    const ack = await this.#o.stream.declare(this.#o.subscriptions);
    if (ack.rejected.length)
      throw new Error(
        `subscription rejected: ${ack.rejected.map((x) => x.topic).join(',')}`,
      );
    ...
    for (const symbol of this.#o.symbols) {
      try {
        ...
        const snapshot = await this.#o.snapshots.getRecoverySnapshot(
          market,
          symbol,
          signal,
        );
        ...
        for (const trigger of this.#o.evaluateRecovery?.(snapshot) ?? [])
          triggers.push({ ...trigger, book: snapshot.book });
      } catch {
        blocked.push(symbol);
        if (this.#o.incidents)
    ...
    if (this.#o.stabilityMs ?? 5000)
      await (this.#o.clock ?? defaultClock).sleep(
        this.#o.stabilityMs ?? 5000,
        signal,
      );
    return {
      market,
