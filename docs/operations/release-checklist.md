...
- [x] A reference-host deploy rejects stale or mixed runtime images before
  migrations, and rejects running containers at another revision before it
  reports success. Evidence: the publish workflow labels every runtime image
  with the source revision; deploy-lib tests cover matching, mismatched,
...
- [x] Graceful deployment preserves `CANCEL_ONLY → old leader disconnect → new
  ...
  Resolved 2026-09-04 (#65, spec §16.54): five of the seven CI drill failures
...
- [x] Trivy 0.74.0 reports zero HIGH/CRITICAL findings for both final images:
  ...
  packages `0`.
- [x] Trivy reports zero HIGH/CRITICAL findings for the third image,
  `strategy-runner`, on its first publish (phase D, #101): Alpine 3.23.5 OS `0`,
  node packages `0` (`Publish images` run 33656031983 on `main` `4217556`,
  2026-09-02T16:42Z; the `main` tag moved only after that clean scan).
