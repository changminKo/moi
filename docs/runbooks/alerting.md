...
| `Moi status FAIL` → `Moi 상태 FAIL` | Readiness ≠ 200 (or the edge unreachable → `ready=000`, fail-closed), runtime ≠ SERVING, a market ≠ NORMAL, or placement disabled. The line shows which. A fail whose *only* cause is a market state is announced once the last `MOI_STATUS_MARKET_WINDOW_TICKS` observations (default 6 = 30 min) hold at least `MOI_STATUS_MARKET_GRACE_TICKS` bad ones (default 2): a single blip that reconnects before the next tick is logged in the journal (`market fail pending (1/2 bad ticks in the last 6)`) but never reaches Discord, while a feed that flaps every other tick is announced on its second bad tick. Once announced, the FAIL stays on the board until the window has been clean for its whole length (`market recovery pending`), so a flapping feed is one FAIL line and a `(fail)` heartbeat, not a stream of FAIL/recovered pairs. Only `DEGRADED`/`RECOVERING` are graced; an `unknown` market state (key missing from `/health/market-data`) fails closed and posts at once, like every other cause, and so does any change once a FAIL has been delivered (a hard cause clearing while the market stays bad is announced at once). While a market recovery is being held, a WARN-level change (memory, swap, disk) waits with it; a new hard FAIL does not. A heartbeat that falls due during a hold is sent with the level Discord is showing (the delivered one), the current line underneath, so its colour can lag the journal by one hold. `MOI_STATUS_MARKET_GRACE_TICKS=1` switches the grace off in both directions. The knobs are read from `/etc/moi/moi.env` (the `EnvironmentFile` of `moi-status.service`). Prometheus (`infra/monitoring/prometheus-alerts.yaml`) fires earlier on the same state — `MarketDataRecoveringSustained` after 2 minutes, `MarketDataDegradedSustained` after 5 — so a Prometheus alert with no Discord line yet is the grace window, not a broken webhook. | Market DEGRADED/RECOVERING → `docs/runbooks/market-data-degraded.md`. Readiness down → `sudo docker compose ... ps`, `journalctl -u moi`. Placement false with markets NORMAL → an incident is open: `GET /admin/incidents`. |
...
| `Moi status heartbeat (<level>)` → `Moi 상태 하트비트 (<level>)` | Nothing changed for 24 hours; the pipeline (timer → check → webhook) is alive. The level and colour are the *delivered* status — what the last FAIL/WARN/recovered line said — which is the current status except during a held market tick (see the FAIL row), so a `(fail)` heartbeat is a day-old outage nobody acted on. | Nothing for `(ok)`; treat `(warn)`/`(fail)` as the original alert. |
...
sudo cat /var/lib/moi/status.last                    # line 1 = last delivered status, line 2 = epoch of that post
sudo cat /var/lib/moi/status.last.grace              # market window: one 0/1 per tick (newest last), epoch of the last tick
