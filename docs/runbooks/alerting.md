...
| `Moi status FAIL` → `Moi 상태 FAIL` | Readiness ≠ 200 (or the edge unreachable → `ready=000`, fail-closed), runtime ≠ SERVING, a market ≠ NORMAL, or placement disabled. The line shows which. A fail whose *only* cause is a market state is posted after it has been observed on `MOI_STATUS_MARKET_GRACE_TICKS` consecutive ticks (default 2, so ≥ 5 minutes): a feed blip that reconnects before the next tick is logged in the journal (`market fail pending (1/2)`) but never reaches Discord. Every other cause posts on first sight. | Market DEGRADED/RECOVERING → `docs/runbooks/market-data-degraded.md`. Readiness down → `sudo docker compose ... ps`, `journalctl -u moi`. Placement false with markets NORMAL → an incident is open: `GET /admin/incidents`. |
...
sudo cat /var/lib/moi/status.last                    # line 1 = last delivered status, line 2 = epoch, line 3 = ticks a market fail has been pending
