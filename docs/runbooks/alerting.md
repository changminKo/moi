...
| `deploy finished: <sha>` | Sent only after readiness 200, both markets NORMAL, placement enabled and every running runtime container at the checked-out revision were observed. A trailing `UNLABELED legacy image accepted` means the operator rolled back to an image published before revision labels with `MOI_DEPLOY_ALLOW_UNLABELED=1`. | Nothing; if the legacy note appears unexpectedly, ask who ran the deploy. |
