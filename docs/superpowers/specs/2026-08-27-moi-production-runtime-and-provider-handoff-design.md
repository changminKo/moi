...
| 16.50 | §7.5 스트림 허브 "세션은 OPENING 으로 먼저 등록되고 … `promoteToLive` 가 큐를 비운 뒤 LIVE 로 바꾼다"; §5.3 "시세는 재생하지 않는다" | **OPENING 창의 quote 가 유실됐다.** `StreamSession.open` 은 `ready` 를 보낸 뒤 DB 에서 계정 이벤트를 재생하고, 허브는 그 뒤에야 엔트리를 LIVE 로 올린다 — 그런데 quote 는 LIVE 엔트리에만 송출됐다. 클라이언트는 `ready` 를 받은 순간부터 연결됐다고 믿고(전략 러너는 그 순간 REST 재기준까지 마친다) 그 창에 시장이 낸 시세는 어디에도 없다: 계정 이벤트는 큐에 쌓여 승격 때 흘러가지만 시세는 재생하지 않으므로(§5.3) **다음 호가 변화까지 아무 가격도 오지 않는다**. 재시작한 러너에서 재현됐다(커서가 있어 replay 가 길다 → 창이 넓다): `ready(6)` 뒤 v7·v8 두 시세가 OPENING 에 떨어져 사라지고 러너는 30초를 기다렸다 — 킬 스위치 통합 테스트가 CI 에서 5회 중 2회 실패한 원인. 고침: 허브가 업그레이드 시점의 요청 심볼(`?instruments=`)을 엔트리에 갖고, OPENING 엔트리에도 그 심볼의 quote 를 소켓으로 바로 보낸다(`quoteFrame` 빌더 하나를 세션과 허브가 공유). LIVE 뒤에는 이전처럼 세션의 구독 집합이 판정한다. `ready` 를 승격 뒤로 옮기는 대안은 재생 이벤트가 `ready` 앞에 오게 되어 클라이언트 계약이 바뀌므로 택하지 않았다. | `stream-hub.test.ts` "forwards quotes for requested symbols to an OPENING entry instead of dropping them"(요청 심볼만, 다른 심볼은 아님); 기존 U13 이 LIVE 팬아웃 유지 확인 |
