...
| 16.42 | 전략 러너 설계 §8.2 "체결은 `SimulatedExchange`(지정가 도달 시 체결, 시장가는 반대편 최우선호가)" | 그 두 규칙에 **현금·포지션·수수료·미체결 주문**을 더한다 — 전략이 돈을 벌었다고만 하고 그 돈이 있었는지는 말하지 않는 보고서는 보고서가 아니다. 그리고 `STOP`·`TAKE_PROFIT`·`OCO`는 조용히 체결되지 않는 대신 **거절한다**: 트리거를 모델링하지 않으면서 영영 체결되지 않게 두면, 보호 주문이 아무것도 하지 않은 전략을 보호가 필요 없던 전략으로 보고하게 되고 그것이 백테스트가 할 수 있는 가장 위험한 말이다. 시장성 지정가는 더 나은 최우선호가가 아니라 **자기 지정가**에 체결된다 — 스냅샷은 호가가 얼마였는지 말할 뿐 얼마나 있었는지는 말하지 않으므로, 가격 개선은 하니스가 지어낸 수가 된다. 깊이도 큐도 없으므로 부분 체결이 없고 대기 순번도 없다(둘 다 전략을 실제보다 좋게 보이게 하는 방향이라 모듈 문서와 README 에 명시한다). 수수료는 §1 13행대로 계획의 것이지 원장의 것이 아니며 §8.3대로 보고서 머리가 그것을 말한다. | `simulated-exchange.test.ts`, `engine.test.ts` |
