...
| 16.33 | 체결 이력을 클라이언트가 재구성할 수 있게 공개 — `ORDER_FILLED` 페이로드 보강 + `GET /api/v1/fills` (#37) | 설계는 계정 스트림만으로 클라이언트가 자기 원장을 따라올 수 있다고 보았으나, `ORDER_FILLED` 페이로드는 `orderId`·`status`·`filledQuantity`만 실어 **체결가·수수료·side·종목을 알 수 없었고**, outbox 재생은 보존 기간에 묶여 있어(`prunePublishedOutbox`) 그보다 오래 끊긴 클라이언트는 과거 체결에 도달할 방법이 아예 없었다. 실현손익·손실 한도·체결 기반 전략이 전부 불가능했다(교차검증 BLOCKER). (1) `fills`에 `session_id`·`account_sequence`·`fill_sequence`(bigserial) 추가(`004_fill_history.sql`). (2) `ORDER_FILLED` 페이로드에 `fills: FillRecord[]`를 **중첩**으로 싣는다 — 발행 시 포트폴리오 스냅샷이 페이로드 위에 병합되므로 최상위 `market`은 스냅샷의 `market` 객체에 덮인다. (3) `GET /api/v1/fills?after=<fillSequence>&limit=` (기본 50, 최대 200, `private, no-store`)로 보존 기간을 넘긴 catch-up을 제공한다. (4) 이벤트와 엔드포인트는 `fillRecord()` 하나를 공유해 같은 체결을 다르게 서술할 수 없다. 커서 근거: `fill_sequence`는 전역 bigserial이지만 한 세션의 모든 쓰기가 `runSessionTransaction`(세션 행 `for update`) 안에서 직렬화되므로 **세션 안에서는 할당 순서 = 커밋 순서**이며, 전역 시퀀스의 갭 가시성 문제가 생기지 않는다. `account_sequence`는 정렬 키가 아니라 스트림 정합용이며, 마이그레이션 이전 체결에는 없으므로 `null`을 그대로 보고한다(값을 지어내지 않는다). | `fill-history.integration.test.ts`(페이징·세션 격리·정확한 금액·null 시퀀스), `portfolio-routes.integration.test.ts`(커서·limit 상한·캐시 헤더·호출자 지정 세션 거부), `production-runtime.integration.test.ts`(발행된 `ORDER_FILLED`가 가격 매길 수 있는 fills를 싣고 `accountSequence`가 이벤트와 일치) |
