...
| `TOSS_CLIENT_ID` | adapter=`toss`일 때 예 | — | 비밀. 정규식 `^[A-Za-z0-9_-]{8,}$` — 계약의 `c_…`는 예시일 뿐이고 실제 콘솔은 다른 접두어(`ts…`)를 발급하므로 접두어를 강제하지 않는다(§16.26) |
...
| 16.14 | §10.2-10 (e) 부분 lease 상태 “0회 관측”; (d) 연결 0 관측 | “연속 500 ms 이상 지속 0회”. KR lock은 backend 사망 즉시 풀리므로 후속자가 KR을 잡는 순간과 패자의 US 해제 커밋 사이 수 ms 창은 제거할 수 없다; 불변식은 교착 부재(§3.11)다. “연결 0 순간” 단언은 100 ms 샘플러가 아니라 가짜 WS의 연결 생명주기 이벤트 로그(`lifecycle`)로 확인한다 — CI에서 해제→재획득 창이 13 ms로 샘플에 잡히지 않아 실패한 뒤 교체. | 드릴 10단계 |
...
| 16.22 | §6.1 조건부 트리거 인터록·§7 원장 시퀀스 (Codex 딥 리뷰 2차, 레인 B) | BLOCKER ①: 트리거 루프가 `isGateExclusive`를 검사하지 않아 RE_ELECTING/DRAINING/RECOVERING 중에도 발동 가능 → 북 매칭과 동일 인터록 추가. BLOCKER ②: fill/trigger 영속화가 세션 row를 잡지 않은 채 `account_sequences`를 `max+1`로 할당(시장 간·요청 간 충돌 시 23505로 트랜잭션 전체 롤백, 엔진은 `TRIGGERED`로 고착) → 공용 `runtime/ledger-transaction.ts`: 세션 `for key share` 선점 후 작업, 40001/40P01 3회 재시도, 시퀀스 할당 헬퍼 공유. HIGH: 트리거 영속 실패 시 엔진이 해당 주문을 `PENDING_TRIGGER`로 되돌리고 같은 체결에 걸린 다른 조건부 주문은 계속 평가, 오류는 루프 종료 후 전파. MEDIUM: OCO 한 다리 취소 = 브래킷 전체 취소(형제 CANCELLED·`oco_groups RESOLVED`·공유 예약 해제·다리별 `ORDER_CANCELLED` 이벤트, reason `USER`/`OCO_SIBLING_CANCELLED`); `OrderRepository.findOcoLegs`(lock-free)로 그룹·다리를 락 순서에 맞게 선점. | `paper-engine.test.ts` interlocks, `production-runtime.integration.test.ts` OCO cancel, lock accounting 프로브 |
| 16.23 | 아키텍처 §fees 시장별 versioned FeeModel | 런타임 수수료 0 하드코딩 제거. `AppConfig.fees`(`FEE_SCHEDULE_VERSION`, `FEE_{KR,US}_{COMMISSION,SELL_TAX}_RATE`; 프로덕션 필수·비프로덕션 기본 v1: KR 0.015%+매도세 0.15%, US 0.25%; compose 리터럴, preflight가 env 오버라이드 거부). RESTORING에서 `publishFeeModelVersions`가 시장별 `fee_model_versions` PUBLISHED 행을 idempotent 기록하고, 같은 버전에 다른 요율이면 `INVARIANT_VIOLATION`으로 부팅 거부. fills는 `fee_model_version_id`를 참조. 반올림 KR 0자리·US 2자리 HALF_UP. BUY 예약의 `estimatedFee`는 동일 모델로 산정(지정가 또는 기준가 기준). | `config.test.ts`, `production-runtime.integration.test.ts` fee schedule |
| 16.24 | §7 체결·취소 선형화 (Codex 딥 리뷰 2차, 레인 A) | BLOCKER ①: 체결(order→reservation→balance)과 취소(balance→order→reservation) 락 순서 역전 → 모든 원장 경로가 `session → wallet/position → oco_group → orders(asc) → reservation`으로 통일(`lockBalances`가 체결·트리거의 첫 락). BLOCKER ②: 취소의 비잠금 선독은 잔고 키 발견용으로만 쓰고, 잠근 뒤 `lockReservation`으로 현재 금액을 재조회해 해제; OCO는 두 다리 모두 종결 후 공유 예약 해제. BLOCKER ③: 취소는 DB 트랜잭션 커밋 후에만 엔진 `cancelOrder`(`runtime/order-cancellation.ts`) — DB 락 보유 중 엔진 await 금지; 체결 영속화는 주문이 이미 종결이면 `OrderTerminalError`로 거부하고 엔진은 그 주문을 CANCELLED로 정리(`engine.fill_superseded`), 그 외 실패는 매칭 전 상태로 복원. HIGH: 정산 산술 `moneyDecimal`+`assertExactMoney`(80자리), 평균단가는 코어 `calculateAverageCost`(10자리). MEDIUM: LIMIT 부분체결 후 예약을 잔여수량×지정가+예상수수료로 재산정해 초과분 반환. MEDIUM: e2e 하네스가 프로덕션 영속화·취소 모듈을 그대로 사용. | `fill-settlement.test.ts`(80자리·가격개선), `production-runtime.integration.test.ts` "fill / cancel linearisation"(부분체결→취소, OCO 동시 취소, 배리어 fill-first / cancel-first), lock accounting 프로브 `lockReservation` |
| 16.25 | Codex 딥 리뷰 3차(레인 A 재검증) — 이전 6건 RESOLVED 확인, 신규 HIGH 4·MEDIUM 5 | (1) 엔진 영속화의 세션 핀을 `for key share`→`for update`(key share는 상호 호환이라 KR/US 동시 체결의 `account_sequence` 충돌을 막지 못함). (2) MARKET 잔량은 사유 무관(`IOC_REMAINDER`/`PRICE_PROTECTION`) 즉시 CANCELLED, 엔진이 실제 terminal reason 기록. (3) `/admin/cancel-all`은 열린 주문마다 정식 취소 모듈 호출(예약 해제·OCO 해소·엔진·outbox), `{cancelled, failed}` 반환. (4) 수수료 publish는 insert-or-ignore 후 존재 행(승자 포함)을 요율·`rounding_mode`까지 검증. (5) `lockBalances`가 양측 모두 wallet→position 순으로 두 잔고를 선점. (6) FX 환전을 UoW로 이관: 세션 for update → 지갑 KRW→USD 자연키 순 → `debitCash/creditCash`(신규 리포지토리 메서드, 프로브) → 감사/시퀀스/outbox. (7) `OrderTerminalError`의 실제 종결 상태를 엔진에 반영(FILLED면 수량 채움). (8) SELL 잔여 노출은 수량만 계산. (9) outbox 발행 payload에 세션 스냅샷 병합(웹 리듀서는 payload를 스냅샷 패치로 적용) — 프로덕션·e2e 공통; e2e 하네스는 병행 시퀀스 대신 프로덕션 발행기로 outbox를 배달. (10) 체결 0·상태 불변 매칭은 `onFill` 생략(`ORDER_FILLED` 노이즈 제거). | `paper-engine.test.ts` terminal handling, `production-runtime.integration.test.ts` "ledger discipline across markets, admin, and FX"(교차시장 동시 체결·cancel-all·FX 역방향 동시·수수료 publish 경합), lock accounting 프로브 `debitCash/creditCash` |
| 16.26 | §5 `TOSS_CLIENT_ID` 형식 | 계약 예시 `c_…`를 정규식으로 강제했으나 실제 발급 ID는 `ts…` 32자. 접두어 제한 제거(영숫자/`_`/`-` 8자 이상), preflight·compose 메시지 동일 조정. | `config.test.ts`, `preflight-deploy.test.mjs` |
| 16.27 | Codex 딥 리뷰 4차 (BLOCKER 2·HIGH 3·MEDIUM 2) | (1) 내구 replay가 비병합 payload를 보내 브라우저가 stale 상태로 시퀀스만 전진하던 결함 → `createOutboxEventSource`에 replay 병합(라이브 발행과 동일한 스냅샷 병합) + 웹 리듀서는 스냅샷 형태가 아닌 payload를 받으면 STALE→REST 재조회. (2) FX 견적 동시 소비 → UoW 안에서 `idempotency.begin(fx-quote:<id>)`로 내구 클레임(세션 핀보다 앞, 락 순서 준수), 완료 시 `complete`; 4중 동시 요청에서 정확히 1회 적용 테스트. FX 라우트는 공개 코드를 계약 상태(`QUOTE_CONSUMED` 409)로 매핑. (3) 스냅샷은 이벤트 시퀀스 “시점”이 아닌 발행 시점 최신 상태 — 웹은 시퀀스 연속성만 검사하고 최신 스냅샷 병합은 무해하므로 **수용된 편차**(9개 조회가 READ COMMITTED에서 찢어질 수 있음은 후속). (4) cancel-all: 재스캔 루프(최대 10회)·실패 잔존 시 503, 내구 명령 레코드는 **미구현(예외)**. (5) `stop()` 꼬리: 타임아웃(`ShutdownBudgetExceeded`)과 실패를 구분 기록, 타임아웃이면 `forced`, 시그널 종료 코드 1. (6) e2e `deliverOnlyLast`는 결함 주입임을 타입·주석으로 명시, e2e 워크스페이스에 `typecheck` 편입. (7) preflight는 production 환경에서 `--skip-egress`/`--egress-ip`/`EGRESS_IP` 거부. | `portfolio-store.test.tsx`, `production-runtime.integration.test.ts` "durable replay and FX claims", `preflight-deploy.test.mjs` |
| 16.28 | Codex 딥 리뷰 5차 (Oracle 호스트·위생 PR) BLOCKER 2·HIGH 4·MEDIUM 6·LOW 2 | (1) `deploy.sh`는 root로 실행(`/etc/moi` root 0700), git/pnpm은 저장소 소유자로 실행; 요청 ref를 fetch 후 detached checkout(ff 실패 무시 제거). (2) `bootstrap.sh` 재실행 안전: `sudo test -f`로 존재 확인, age 키는 root 임시파일에 생성 후 원자적 설치, 기존 키·moi.env 절대 덮어쓰지 않음; Node 정확 버전(≥24.19) 검사. (3) 스냅샷 병합용 REPEATABLE READ UoW 호출을 `#uow.track`으로 감싸 종료 드레인에 포함. (4) 배포 검증은 readiness + 두 시장 `NORMAL` + `placement:true`를 요구, 미충족 시 실패. (5) 시크릿이 Docker 컨테이너 메타데이터(root만 열람)에 남는 노출을 **수용 편차**로 문서화(파일 기반 시크릿은 후속). (6) 공개 엣지에서 `/metrics` 404, HSTS 헤더. (7) preflight가 `PUBLIC_ORIGIN`/`PUBLIC_API_ORIGIN`과 `WEB_DOMAIN`/`API_DOMAIN` 일치 요구. (8) 새 이미지로 사전 마이그레이션 잡(`dist/migrate-cli.js`)을 구 스택 가동 중 실행. (9) cancel-all 감사에 서버측 `requestId`·`actor=ADMIN_API_KEY`, 본문 actor/reason은 untrusted 메타데이터로 기록; 테스트는 commandId로 상관. (10) 배포 가이드의 stale "release blocker" 문구 제거. 미해결→이슈: 스냅샷 격리/재시도 테스트(#11), cancel-all 내구 명령(#10). | `preflight-deploy.test.mjs`, `production-runtime.integration.test.ts` cancel-all, `bash -n`, 호스트 첫 배포 |
| 16.29 | Oracle 참조 호스트 TLS 엣지를 **단일 오리진**으로 변경 (`infra/oracle/Caddyfile`) | 설계는 `app.<domain>`/`api.<domain>` 두 호스트를 전제했지만, 무료 DNS(DuckDNS)는 Public Suffix List에 있어 두 서브도메인이 서로 다른 *사이트*가 되고 `SameSite=Lax` 세션 쿠키가 API에 전달되지 않는다(첫 배포에서 인증 필요 API 전부 401). Caddy가 한 호스트에서 `/api/*`·`/health/*`·`/admin/*`을 paper-api로, 나머지를 web으로 경로 라우팅한다; `/metrics`는 여전히 404. `API_DOMAIN`은 `WEB_DOMAIN`과 같은 값으로 두어 preflight 교차검증과 deploy.sh 검증 경로를 그대로 쓴다. 쿠키 정책(`SameSite=Lax`)과 CORS 코드는 변경하지 않는다 — 같은 오리진에서는 둘 다 무관해진다. 유료 도메인으로 옮겨도 이 배치를 유지한다(단순·Safari 서드파티 쿠키 차단 무관). | `caddy validate`, 호스트 배포 후 브라우저에서 세션·지갑·WebSocket 확인 |
| 16.30 | 운영 알림(Discord) — 스펙 외 운영 추가 | 호스트에 `moi-status.timer`(5분, `status-check.sh`: readiness·runtime·KR/US 상태·placement·메모리·스왑·디스크를 한 줄로 축약, **전달된** 마지막 줄과 다를 때만 게시, 24h 무변화 시 heartbeat), `deploy.sh` 시작/완료/실패 알림(EXIT/INT/TERM/HUP 트랩, 검증 단계가 `VERIFIED=1`을 세운 뒤에만 성공 보고, 배포 중 `/run/moi-deploy.lock`으로 타이머 침묵), `OnFailure=moi-alert@%n.service`(oneshot 시작/중지 실패만 — 컨테이너 사망은 타이머가 포착). `notify.sh`는 기본 fail-open(웹훅 부재·게시 실패 시 exit 0), status-check 내부에서만 `NOTIFY_STRICT=1`로 재시도; 제목·본문은 자격증명 마스킹 후 전송(규칙 2). CI/publish 결과는 `notify.yml`(`workflow_run`)이 게시. 런타임 동작 변경 없음. | `infra/oracle/status-check.test.mjs`(status·notify·deploy-lib), `check-deployment-contract` 웹훅 URL 스캔 |
| 16.31 | 시장 세션(장 상태)을 클라이언트에 공개 — `GET /api/v1/markets/{market}/session` | 설계에는 캘린더를 노출하는 공개 엔드포인트가 없어, 브라우저가 장 상태를 알 수 있는 유일한 경로가 주문을 넣어 `MARKET_CLOSED` 거절을 받는 것뿐이었다. 웹이 "장 마감 · 09:00 개장"처럼 정직한 안내를 하려면 상태가 필요하다. 응답은 `{market, phase, isTradingDay, opensAt, closesAt, asOf, serverTime}`, `phase ∈ REGULAR|PRE_OPEN|POST_CLOSE|CLOSED|HOLIDAY`. phase는 순수 함수 `derivePhase(day, now)`가 서버 시각으로 매 요청 파생하며(개장 순간은 REGULAR, 폐장 순간은 POST_CLOSE), 캐시는 **날짜(day)만** 보관해 세션 경계를 넘겨 굳지 않는다(기존 `MarketCalendarService`는 REGULAR/CLOSED를 영구 캐시했다). 거래일은 시장 현지 시간대(KR Asia/Seoul, US America/New_York)로 계산한다. 인증 없는 공개 참조 데이터 (`Cache-Control: public, max-age=60`), 캘린더 조회 실패 시 phase를 지어내지 않고 503 `SERVICE_UNAVAILABLE`. `ProviderBundle.calendar`로 배선(toss=`TossRestClient`, fake=신규 `FakeCalendarSource`); 주문 경로의 `MARKET_CLOSED` 판정과 `session` 필드 의미는 변경하지 않는다. | `market-session.test.ts`(경계·휴장·미상 윈도), `market-session-routes.test.ts`(200 shape·400·503·phase 전이), `market-calendar-port.test.ts`(현지 거래일), `market-calendar-service.integration.test.ts`(파생·TTL·스탬피드) |
| 16.32 | `@moi/strategy-sdk`가 페이퍼 API의 실제 응답을 해독하지 못하던 계약 드리프트 정정 (#32) | SDK가 원장 내부 타입(`OrderSnapshot`/`WalletSnapshot`/`PositionSnapshot`)을 와이어 페이로드로 재사용해, 응답에 없는 `version`을 요구하고(성공한 주문이 `INVARIANT_VIOLATION`으로 실패) API가 보내는 `market`·`averageCost`는 버렸다. SDK 소유 와이어 타입(`BrokerOrder`/`BrokerWallet`/`BrokerPosition`/`BrokerPortfolio`)을 도입해 페이로드를 그대로 반영한다. `version`은 공개하지 않는다 — 어떤 쓰기도 클라이언트 버전을 받지 않고(동시성은 `Idempotency-Key`), 클라이언트가 관측하는 순서는 `accountSequence`다. 함께 정정: 조건부·OCO 주문 와이어(SDK `triggerPrice` → API `stopPrice`, 평면 OCO → 2-leg `legs`), 부재의 표기(`terminal_reason`만이 nullable 컬럼이라 API가 `null`로 보내는 유일한 필드인데 이전 디코더는 `undefined`만 허용해 모든 포트폴리오 조회가 실패 — 나머지 필드의 `null`은 여전히 malformed), 조회 행과 쓰기 응답의 분리(`BrokerPortfolioOrder`: market·symbol·type·side·quantity·prices·fills·siblingOrderIds까지 그대로 — 전략이 자기 주문을 식별하려면 필요하다; 쓰기 응답은 `{id, status, quantity, filledQuantity}`로 다른 페이로드다), `activeOrders`에 상태 필터가 없다는 서버 실제 동작을 페이크와 통합 테스트 양쪽에 고정(#33은 #37 이후로 미룬다 — 이 행들이 현재 체결 데이터에 도달하는 유일한 경로다), 포지션 중복 판정 키를 원장과 같은 `(market, symbol)`로 정정, 401·403 분류(각각 `SESSION_EXPIRED`·`FORBIDDEN`으로 분리, 둘 다 `DomainErrorCode`에 추가 — 공개 에러 계약에는 처음부터 있었다. CSRF 토큰 누락은 403인데 이전에는 계정이 read-only라고 보고했다), `PaperBrokerPath` 주석의 과장(경로만 제한하며 호스트는 트랜스포트가 정한다). API 측 변경은 하나: `/api/v1/portfolio`가 `sessionId`를 반환한다(클라이언트가 자기 세션의 계좌인지 확인할 수 있어야 하고, 값은 호출자가 이미 가진 것이다). 스트림 패치(`#enrichPayload`)도 같은 필드를 실어 REST 스냅샷과 형태를 일치시킨다 — 두 형태가 갈라지는 것이 이 드리프트의 원인이었다. | `apps/paper-api/src/runtime/paper-broker-contract.integration.test.ts` — 실제 런타임·실제 원장을 상대로 `PaperBroker`가 MARKET/LIMIT/STOP/TAKE_PROFIT/OCO 배치·취소·포트폴리오·멱등 재생·401을 왕복한다. 페이크 트랜스포트끼리만 합의하던 구조가 드리프트의 원인이었다. |
| 16.33 | 심볼 상태 슬롯이 체결과 호가를 서로 덮어쓰던 결함, 그리고 quote가 §16.17의 "현재 호가"에서 "현재가(마지막 체결)"로 바뀐 것 | `MarketStateStore`는 심볼당 슬롯이 하나인데 `#dispatch`가 체결 payload(`{market,symbol,price,sourceTimestamp}`)와 호가 payload(`OrderBookSnapshot`)를 같은 슬롯에 **모양이 다른 채로** 번갈아 써서, 나중에 온 프레임이 앞의 것을 지웠다. `#quote`는 호가 모양만 읽었으므로 마지막 프레임이 체결이면 `price:null`을 돌려줬다 — 프로덕션에서 AAPL이 2초 간격 10회 연속 null, 6종목 중 5종목 null로 관측됐다(2026-09-01). 같은 값이 `OrderPlacementService.referencePrice`였기 때문에 **시장가 매수가 프레임 도착 순서에 따라 `MARKET_DATA_DEGRADED`로 거부**됐다(`market-data`가 정상이고 `placement:true`인데도). 확정: 슬롯은 `SymbolQuoteState = {book?, lastTrade?}` **한 가지 모양**만 담고 두 생산자가 병합해 쓴다(엔진 envelope에는 종전대로 각자의 원본 payload를 넘긴다 — 엔진 계약 불변). 읽기는 두 갈래로 분리한다: `quotePrice`(표시용) = 마지막 체결 → 최우선 매도호가 → 최우선 매수호가, `referencePrice`(MARKET BUY 예약 산정) = **최우선 매도호가 우선** → 마지막 체결 → 매수호가. 표시용이 §16.17의 "현재 호가"에서 벗어나는 편차이며, 예약 산정은 ask 기준을 유지해 원장 보수성을 바꾸지 않는다(체결가를 쓰면 호가가 위로 벌어졌을 때 과소 예약이 된다). REST 복구 baseline은 자기 `book`을 통해 읽히므로 이후 라이브 호가가 스냅샷을 이긴다. | `symbol-quote-state.test.ts`(병합·우선순위·baseline), `market-runtime.test.ts` "keeps the book and the last trade in one slot whatever the frame order"(원본 코드에서 RED)·"hands the engine the raw trade and book payloads", `order-placement-service.test.ts`(두 프레임 순서 모두에서 MARKET BUY가 ask로 산정되고 거부되지 않음), `production-runtime.integration.test.ts` 기존 quote 단언 |
| 16.34 | §8.3 “5분 슬라이딩 창에서 3회 실패 시 자동 재시도 중단 + 수동 incident `RECOVERY_RETRY_EXHAUSTED`” · §8.2 재연결 | 34시간 시장 데이터 장애의 원인 두 건을 함께 고친다. **(a) 소진은 정지가 아니라 감속이다.** `ReconnectSupervisor`가 `#exhausted`를 영구 래치로 써서 `schedule()`이 첫 줄에서 되돌아왔고, 유일한 해제 경로가 관리자 라우트의 `resumeRetries()`였다 — 프로덕션 프로세스는 Toss 소켓 0개로 21시간을 조용히 보냈다. 소진 뒤에도 `schedule()`은 동작하되 지연을 `rearmDelayMs`(30 s부터 배증, 5 min 상한, 각 단계의 상위 절반에서 균등 추출 — full jitter가 아닌 half jitter인 이유는 하한이 0에 붙으면 상한을 둔 의미가 사라지기 때문이고, 지터가 필요한 이유는 KR·US가 같은 provider 사건으로 함께 소진되면 고정 지연이 두 시장의 재시도를 영구히 겹치게 하는데 둘이 공유하는 `OAuthTokenProvider`의 `TOKEN_MIN_REISSUE_INTERVAL_MS` 10 s 하한이 두 번째 호출자에게 `AUTH_THROTTLED`를 돌려주기 때문이다 — 한 시장의 실패가 다른 시장의 실패를 만들어낸다)로 바꾼다; `onExhausted`는 **진입 전이에서 한 번만** 울리고(재무장 실패마다 incident를 복제하지 않는다), `exhausted`는 경보 상태로 남는다. 재무장한 시도가 성공하면 `reset()`이 홀드를 풀고 `MarketRuntime`이 그 시장의 `RECOVERY_RETRY_EXHAUSTED` incident를 `resolveRetryExhausted`로 해제한다 — 그러지 않으면 피드가 돌아와도 시장은 `CANCEL_ONLY`에 남는다. (**16.35에서 해소됨**: 예고한 대로 `MarketIncidentPort.resolveRetryExhausted`·`#buildMarket` 어댑터·`#clearRetryHold()`의 호출을 제거했고, 홀드를 푸는 `RECOVERY_RETRY_EXHAUSTED` 행 해소는 16.35의 일반 정책(`OPERATOR_ONLY_CAUSES` 기반, 어느 프로세스가 연 행이든 원장에서 해소)이 맡는다 — 이 포트의 트리거는 인메모리 `supervisor.exhausted`라 재시작한 프로세스에서는 호출조차 되지 않고 대상도 cause code 하나뿐이었다. `#clearRetryHold()`는 `supervisor.resume()`과 `recovery.hold_cleared` 로그만 남기고 동기 함수가 됐다 — 재시도 홀드는 supervisor 자신의 생명주기이므로 여기 남고, `wasExhausted` 게이트도 유지한다(홀드가 없었는데 `recovery.hold_cleared`가 남으면 로그가 거짓 신호가 된다). await 하나가 사라지면서 그 try/catch는 던지지 않는 것만 감싸게 되어 함께 제거했다. 함께: 포트의 `manual?: boolean`은 `#buildMarket` 어댑터가 버리고 있던 죽은 필드라 삭제했다 — `source = 'MANUAL'`은 cause code로부터 `MANUAL_CAUSES`가 붙인다.) 두 절반은 함께여야 한다 — 16.35의 자동 해소는 소진 상태에서 완료되는 복구가 있어야 도달하고, 그 복구를 다시 일어나게 하는 것이 이 행의 재무장이다. `resume()`(운영자)은 대기 중인 재무장 타이머를 취소해 즉시 재시도한다. incident의 `source = 'MANUAL'`(§`MANUAL_CAUSES`)은 유지한다 — 사람이 봐야 할 사건이라는 표시일 뿐, 이제 복구의 **유일한** 경로는 아니다. 프로세스 단위 supervisor의 lease 재획득 루프는 소진 시 계속 던진다(`#acquireBundle`) — 동작 변경 없음. **(b) 교체된 소켓을 떼어낸다.** `handshake()`가 `this.socket`을 덮어쓰고 `this.closed = false`로 되돌리면서 이전 소켓을 닫지도, 그 `onclose`/`onmessage`/`onerror`를 떼지도 않았다. 죽은 소켓의 늦은 `onclose`가 `finish()`를 호출해 새 연결의 제어 대기자를 `{kind:'error', code:'transport-closed'}`로 깨웠고(`declare()`에는 `SUBSCRIPTION_REJECTED: declaration rejected: transport-closed`로 보였다), 버려진 소켓은 provider에 등록된 채 남아 다음 연결이 **중복 구독**이 되어 `"Bye"` 축출 → 재연결 → 다시 중복이라는 자기증폭 루프가 됐다(13시간 81회). 이제 `detachSocket()`이 핸들러를 `delete`하고 소켓을 닫은 뒤에야 새 소켓을 세우며, 소켓마다 세대 토큰을 부여해 핸들러가 자기 세대가 현재일 때만 동작한다(핸들러 참조를 이미 붙잡은 트랜스포트에 대한 두 번째 방어). 미읽은 이벤트 큐도 handshake에서 비운다 — 끝난 epoch의 `transportClosed`가 새 연결을 곧바로 degrade 시켰다. `close()`도 같은 detach를 쓴다. abort 리스너는 `close()`보다 먼저 settle 해 abort가 `AbortError`로 남게 한다. 진행 중인 handshake의 소켓을 다음 handshake가 떼어가면 그 promise는 `abandonHandshake`로 `TRANSPORT_CLOSED`(superseded) 거부된다 — 호출자 규율에 기대지 않기 위한 것이다 (`MarketRuntime.#recoverOnce`의 `#recovering` 가드가 지금은 동시 `connect()`를 막지만 이 어댑터는 범용이다). handshake의 큐 비우기는 “`events()` 소비자가 정확히 하나이고 순서대로 다 비운다”는, 이 클래스가 강제할 수 없는 불변식에 기대므로 코드에 명시해 두었다. | `reconnect-supervisor.test.ts`(재무장, 상한을 지터 밴드 양끝에서 고정 — `random: () => 1`이면 `[30_000, 60_000, 120_000, 240_000, 300_000, 300_000]`, `random: () => 0`이면 `[15_000, 30_000, 60_000, 120_000, 150_000, 150_000]` — 같은 사건으로 소진된 두 scope의 지연 분리, 성공 시 해제), `market-runtime.test.ts` “keeps retrying after the hold and clears it without an operator”, `toss-websocket.reconnect.test.ts`(늦은 `onclose`·늦은 프레임·`close()` 후 재연결·superseded handshake 거부 — 이 마지막 건은 codex 리뷰에서 “정확하지만 테스트 0건”으로 지적받아 추가했고, `abandonHandshake?.()`를 빼면 5 s 타임아웃으로 hang 한다) |
| 16.35 | §6.2/§6.4 시장 헬스 상태와 배치 게이트의 단일 진실 (§16.34 재무장에 의존) | 34시간 시장데이터 장애의 복구를 어렵게 만든 결함 두 건. (a) `MarketHealthMachine.degrade()`가 `#incident` 검사와 대입 사이에 `await`를 두어, keepalive 핑과 이벤트 루프가 같은 시장을 동시에 degrade 하면 행이 두 개 열리고 하나는 아무도 추적하지 않아 영구 `ACTIVE`로 남았다(실제 고아: US `PONG_FAILED`). 두 번째 호출은 진행 중인 activation에 합류한다(degrade 1회당 incident 1건은 유지). (b) `state`는 인메모리 헬스 머신에서, `reasons`·`placement`는 DB의 `ACTIVE` 행에서 나왔다. 헬스 머신은 프로세스와 함께 죽고 행은 살아남으므로, 재시작 직후 두 시장 모두 `state=NORMAL`인데 `placement=false`가 되고 운영자가 보는 곳 어디에도 이유가 없었다. 두 가지를 고친다: `markHealthy(epoch)`가 이 시장이 소유한 `ACTIVE` 행 전체를 해소하고(어느 프로세스가 열었든 — `IncidentPort.resolveMarketIncidents`), `/health/market-data`의 시장 상태를 `reasons`와 같은 행에서 파생한다(`runtime/market-health-view.ts`: PLACE를 막는 행이 하나라도 있으면 `NORMAL`이 될 수 없고, GLOBAL 행은 두 시장 모두에 적용). `MANUAL_CAUSES`(DB `source='MANUAL'` 표기)에서 자동 해소 제외 집합 `OPERATOR_ONLY_CAUSES`를 분리하고 `RECOVERY_RETRY_EXHAUSTED`를 후자에서 뺀다 — 그 행이 주장하는 "재시도 소진"은 방금 성공한 복구가 반증한 사실이며, 전송이 명백히 살아 있는 시장의 배치를 계속 막았다. `STARTUP_INVARIANT_OR_AUDIT_FAILURE`는 남긴다(원장 무결성은 소켓이 낫게 하지 못한다). 재시도 홀드 해제는 supervisor 자신의 성공 경로(§16.34 `MarketRuntime.#clearRetryHold`)가 맡고 여기서는 행만 해소한다. 해소는 `resolved_by`에 주체를 남긴다 — 복구는 `RECOVERY`, 운영자 라우트는 `ADMIN_API`(이전에는 두 경로 모두 `runtime`이었다). `MANUAL`로 열린 행이 사람 없이 닫힐 수 있게 됐으므로 `(source, resolved_by)` 쌍이 그 구분을 짊어진다 — 스키마 변경은 없다, `resolved_by` 컬럼은 처음부터 있었고 값이 무의미했을 뿐이다. 원장 오류로 해소가 실패해도 방금 성공한 복구를 되돌리지 않는다(로그 `incident.recovery_resolve_failed`). **§16.34 의존**: `RECOVERY_RETRY_EXHAUSTED` 자동 해소의 논거("완료된 복구가 재시도 예산 소진을 반증한다")는 §16.34 의 재무장이 있어야 성립한다. 그 전의 `ReconnectSupervisor`는 `#exhausted`를 영구 래치로 써서 `schedule()`이 첫 줄에서 되돌아왔고, 소진된 시장에서는 `#recoverOnce`가 다시 불리지 않아 `markHealthy`에 도달하는 복구 자체가 없다 — 이 행의 자동 해소는 그 시나리오에서 죽은 코드다. 재시작 경로(어제의 실제 경로, 새 프로세스가 부팅 복구에 성공)는 supervisor가 새것이라 §16.34 없이도 동작한다. 두 행은 함께 머지되어야 하며 §16.34 가 먼저다. **후기(#64, 2026-09-04)**: `StartupIncident.activate` 의 `manual: boolean` 도 #60 이 지운 `MarketIncidentPort` 의 플래그(§16.34 후기)와 같은 죽은 플래그였다(프로덕션 어댑터가 `{ causeCode }` 만 읽어 버림) — 제거. MANUAL 여부는 `ProductionRuntime` 의 `MANUAL_CAUSES` 가 리포지토리에 `manualCauseCodes` 로 주입되어 하나로 결정한다. 증거: `startup-coordinator.integration.test.ts`(정확한 인자 `{ causeCode }`·호출 1회), A7(`source: 'MANUAL'`). | `health-machine.test.ts`(경합 degrade·재시작 후 해소·남은 차단 보고), `market-health-view.test.ts`, `production-runtime.integration.test.ts` "incident resolution and the placement gate" |
| 16.36 | §7.5 `quote` 프레임의 payload 모양, 그리고 §16.17 REST quote 투영 | `quote` 프레임의 payload 모양을 아무 문서도 고정하지 않은 탓에 세 계층이 서로 다른 모양을 믿었다. 프레임은 `MarketRuntime.#dispatch`가 `event.book`을 그대로 실어 **`OrderBookSnapshot`**(`{symbol,market,currency,bids,asks}`, 수량 필드는 `volume`)이었고, REST `#quote`는 `{market,symbol,price,asOf,health,recoveryEpoch,marketDataVersion}`으로 **호가를 아예 담지 않았고**, 웹의 `QuoteSnapshot`은 그 둘의 합집합인데 수량을 `size`로 적었다. 웹 훅이 `setQuote(message.payload as unknown as QuoteSnapshot)`으로 **캐스트**해 이 불일치가 컴파일에서 걸리지 않았고, `order-book.tsx`의 `Decimal.max(m, x.size)`가 `undefined`를 받아 렌더 중 동기적으로 던졌다 — `apps/web/src`에 `ErrorBoundary`가 없어 호가 패널이 아니라 **React 트리 전체가 언마운트**됐다(2026-09-01 프로덕션, `/trade?symbol=AAPL`, `pageerror: [DecimalError] Invalid argument: undefined`, `root children: 0`). 34시간 시장 데이터 장애(§16.34) 동안 US 호가 프레임이 오지 않아 드러나지 않았고, 피드가 살아나자 바로 터졌다. 확정: **수량 필드는 전 계층 `volume` 하나**다(원장 컬럼 `book_level_volume`, `@moi/trading-core`의 `OrderBookLevel`, 엔진, 와이어가 이미 쓰는 낱말이며 `size`는 웹에만 있었다) — 웹 `BookLevel`을 `{price, volume}`으로 고친다. **프레임과 REST 스냅샷은 한 모양이다** — 관례가 아니라 구조로. `market-data/quote-projection.ts`의 `projectQuote`가 **유일한 빌더**이고 `MarketRuntime.#publishQuote`와 REST `#quote`가 둘 다 그것을 호출한다(예전엔 객체 리터럴 두 개가 따로 있었고, 그 둘이 벌어진 것이 이 결함이다). `price`는 `quotePrice`(§16.33), 나머지는 `asOf`·`health`·`recoveryEpoch`·`marketDataVersion`·`currency`·`bids`·`asks`다. 모양을 `docs/api/quote-contract.md`에 적고 `QUOTE_PROJECTION_FIELDS`와 테스트로 같게 유지한다 — `error-contract.md`와 `PUBLIC_ERROR_CODES`가 이미 쓰는 방식이다. 그 문서가 없었다는 것이 이 결함의 근인이다 — `#enrichPayload`가 portfolio에 대해 이미 말하는 규율(스냅샷과 패치가 두 모양이면 안 된다, §16.32)을 quote에도 적용한 것이다. 발명은 없다: 슬롯이 비면 `price: null`이고 호가는 **필드째로 생략**해(체결이 첫 호가보다 먼저 와도 클라이언트가 보고 있는 깊이를 지우지 않는다) `asOf`는 §16.17처럼 투영 시각이다. **체결도 프레임을 발행한다** — `quotePrice`가 마지막 체결을 우선하므로 체결이 표시가를 움직이는데 종전에는 호가 이벤트만 발행해서, 패널의 차트(`use-quote-ticks.ts` + `sparklineGeometry`, 2점부터 그린다)가 REST 첫 점 하나로 영구히 `Collecting…`에 머물렀다(보고된 "차트가 없다"의 실제 원인이며 크래시의 증상이 아니다). 웹은 캐스트 대신 경계에서 좁힌다: `lib/quote-frame.ts`의 `applyQuoteFrame(current, frame)`이 payload를 narrow 해 **화면의 quote 위에 병합**하고(호가 프레임이 가격을 재진술하지 않으면 스냅샷의 가격이 남는다), 좁힐 수 없는 프레임은 `null`로 **버려** 마지막 정상 quote가 남는다; 레벨은 `price`·`volume`이 모두 문자열인 것만 통과하고(숫자 가격은 거부 — API는 금액을 십진 문자열로 말한다), `depthPercent`·`maxVolume`은 `format-number.ts`가 렌더 경계에 대해 말하는 계약(십진수가 아니면 그대로 통과, 던지지 않는다)을 지켜 깨진 레벨을 깊이 0으로 그린다. e2e 하네스도 함께 고친다 — 이 결함을 e2e가 놓친 것은 우연이 아니라 하네스가 프로덕션의 대역이 아니었기 때문이다: `injectBook`이 호가를 엔진에 직접 넘기고 `publishQuote`를 **한 번도 부르지 않아** 브라우저에 `quote` 프레임이 간 적이 없고, 가짜 REST quote는 프로덕션이 서브하지 않는 모양(호가를 담고, 그것도 웹의 `size` 표기로)으로 답해 웹의 잘못된 믿음을 그대로 인코딩했다. 배포 순서상 둘로 나눠 넣었다: 죽어 있는 화면을 먼저 살리려고 **웹 최소 수정**(`volume` 통일 · `applyQuoteFrame` · 렌더 총함수화 + 그 단위 테스트)이 먼저 머지·배포되고, 이 행의 서버 정규화·`docs/api/quote-contract.md`·하네스 보강이 뒤따랐다 — 웹 절반은 종전 서버(호가만 실은 프레임)에서도 그대로 옳다(스냅샷의 가격이 남고 호가가 그려진다). `ErrorBoundary` 부재는 이 행에서 고치지 않았다(별건, 아래 후속). | `order-book.test.tsx`(포착한 프레임 렌더 · 깊이 스케일 · 누락/파싱불가 수량이 깊이 0 · `depthPercent`·`maxVolume` 전역성), `quote-frame.test.ts`(포착한 프레임 병합 · 프레임이 말한 가격/`asOf` 채택 · 숫자 가격 거부 · 좁힐 수 없는 payload는 `null` · 다른 종목의 가격을 물려받지 않음 · 레벨 narrowing), `quote-stream.test.tsx`(와이어 모양으로 교체한 픽스처 · 언마운트 없이 호가 렌더 · 프레임당 틱 누적으로 스파크라인 생성 · 호가만 있는 프레임은 틱을 만들지 않음), `quote-projection.test.ts`(단일 빌더의 가격 우선순위·빈 슬롯의 `price: null`·호가 필드를 비우지 않고 생략·버전 필드 문자열화, 그리고 `docs/api/quote-contract.md`가 `QUOTE_PROJECTION_FIELDS`와 정확히 같은 필드를 적는지), `market-runtime.test.ts` "states the price and the instant on the quote frame, not just the book"·"publishes a quote frame per trade even with no book yet"(원본 코드에서 RED), `production-runtime.integration.test.ts` quote 단언에 `currency`·`bids`·`asks` 추가, `quote-stream-panel.spec.ts`(e2e: 스트림 호가 렌더 + `pageerror` 0건 · 두 번째 가격에 차트 생성 · `size`로 잘못 적힌 레벨에도 생존) |
| 16.37 | 체결 이력을 클라이언트가 재구성할 수 있게 공개 — `ORDER_FILLED` 페이로드 보강 + `GET /api/v1/fills` (#37) | 설계는 계정 스트림만으로 클라이언트가 자기 원장을 따라올 수 있다고 보았으나, `ORDER_FILLED` 페이로드는 `orderId`·`status`·`filledQuantity`만 실어 **체결가·수수료·side·종목을 알 수 없었고**, outbox 재생은 보존 기간에 묶여 있어 그보다 오래 끊긴 클라이언트는 과거 체결에 도달할 방법이 없었다. 실현손익·손실 한도·체결 기반 전략이 전부 불가능했다(교차검증 BLOCKER). (1) `fills`에 `session_id`·`account_sequence`·`fill_sequence` 추가. (2) `ORDER_FILLED` 페이로드에 `fills: FillRecord[]`를 **중첩**으로 싣는다 — 발행 시 포트폴리오 스냅샷이 페이로드 위에 병합되므로 최상위 `market`은 스냅샷의 `market` 객체에 덮인다. (3) `GET /api/v1/fills?after=<fillSequence>&limit=` (기본 50, 최대 200, `private, no-store`). (4) 이벤트와 엔드포인트는 `fillRecord()` 하나를 공유한다. **마이그레이션은 004~007로 나눈다**: 004는 카탈로그 전용 DDL과 트리거, 005는 백필, 006은 인덱스와 `not valid` 제약, 007은 검증. `session_id`는 **nullable**이다 — `deploy.sh`는 이전 릴리스가 트래픽을 받는 동안 마이그레이션하고 그 릴리스는 이 컬럼 없이 fill을 넣으므로, NOT NULL이면 마이그레이션 커밋부터 새 이미지 재시작까지 모든 체결이 실패하고 롤백도 깨진다. 대신 `BEFORE INSERT` 트리거가 주문에서 `session_id`를 유도해 옛 릴리스의 fill을 **소유된 행으로** 만든다(단순 허용이 아니라 조회에 보이고 FK도 만족). NOT NULL은 옛 릴리스가 사라진 뒤 별도 릴리스의 몫이다. `fill_sequence`는 `bigserial`이 아니라 `bigint` + 별도 시퀀스 + 기본값이다 — `add column ... bigserial`은 volatile 기본값이라 테이블 전체를 AccessExclusiveLock 아래에서 재작성한다(실측: `relfilenode` 변경). 백필은 `order by occurred_at, id`로 결정적으로 번호를 매긴다 — 힙 순서로 매기면 앞선 UPDATE가 튜플을 뒤로 옮겨 **과거 체결이 최신 체결보다 뒤 번호를 받는 역전**이 생기고, 그 커서로 손익을 재생하면 매수보다 매도를 먼저 기록한다. 커서 근거: 한 세션의 모든 쓰기가 `runSessionTransaction`(세션 행 `for update`) 안에서 직렬화되므로 **세션 안에서는 할당 순서 = 커밋 순서**다. 세션 간 순서는 주장하지 않는다. `account_sequence`는 정렬 키가 아니라 스트림 정합용이며 마이그레이션 이전 체결에는 `null`이다. **남은 한계(측정치)**: Kysely 마이그레이터는 대기 중인 마이그레이션 전체를 하나의 트랜잭션으로 실행하므로(`migrator.js:445`), 파일을 나눠도 **한 배포 안에서는** 004가 잡은 AccessExclusiveLock이 전체 실행 동안 유지된다. `fills` 독자 정지 시간 = 마이그레이션 총 시간이며 행 수에 선형이다 — 100행 10 ms, 3만 행 0.67 s, 30만 행 7.6 s (postgres:17-alpine, 로컬 컨테이너). 모든 파일에 `lock_timeout = 3s`를 걸어 획득 대기가 트래픽 뒤에 쌓이지 않게 했다. 테이블이 커지면 `disableTransactions` 또는 릴리스 분할이 필요하며, 이는 전 마이그레이션에 영향을 주는 정책 결정이라 이 작업 범위 밖으로 남긴다. | `fill-history-migration.integration.test.ts`(백필·occurred_at 순서·재작성 없음 — `bigserial`로 되돌리면 실패하는 것을 확인), `fill-history.integration.test.ts`(옛 릴리스 삽입 호환·FK 위반 거부·페이징·세션 격리·정확한 금액·밀리초·recovery 표시), `portfolio-routes.integration.test.ts`(커서·limit 상한·범위 초과 커서 400·캐시 헤더), `production-runtime.integration.test.ts`(발행된 이벤트의 fills와 `accountSequence` 일치), `@moi/e2e` 20/20 |
| 16.38 | 전략 러너 설계 §8.1 "상태 파일: … `cursor.json`" | **`cursor.json`을 만들지 않는다.** 체결 커서는 `fills.ndjson`의 fold(`max(accountSequence)`)다. §6.4는 이벤트 처리와 커서 전진을 **같은 트랜잭션**에 기록하라고 요구하는데, append-only NDJSON 위에는 트랜잭션 매니저가 없으므로(§8.1이 `node:sqlite`를 배제했다) 원자성의 단위는 **한 줄**뿐이다 — 한 이벤트의 결과 전체(체결·포지션·실현손익·결정 id·새 커서)를 한 레코드로 `write` 한 번, `fsync` 한 번에 쓰고, 꼬리가 찢어진 레코드는 리더가 버린다. 커서를 별도 셀로 두면 저널이 이미 가진 사실의 **두 번째 사본**이 되고, 크래시 뒤 둘이 "이 이벤트를 처리했는가"를 두고 어긋날 수 있다. 어긋나는 방향 양쪽 다 나쁘다: 커서가 앞서면 체결이 어떤 전략에도 닿지 않고(누락), 뒤지면 재생이 중복 발주를 낸다 — §6.4가 막으려는 바로 그것이다. 같은 논거로 포지션도 `runtime.json`이 아니라 이 줄에 싣는다(커서보다 체결 하나 앞선 포지션은 재생이 이미 반영된 체결을 다시 반영하게 만든다). | `fill-journal.test.ts`(찢어진 꼬리 폐기·중간 손상 fail closed·역행 커밋 거부), `fill-journal.partial-write.test.ts`(짧은 쓰기에도 커서가 파일과 같은 자리에 남음), `strategy-runner-stream.integration.test.ts`(SIGKILL 주입 2건) |
| 16.39 | 전략 러너 설계 §5.2 "`Tick.price`는 중간가 또는 REST 스냅샷 가격, 시장별 호가 단위로 반올림" | **중간가를 유도하지 않는다.** §5.2가 중간가를 요구한 이유는 §1 행 3이 적어둔 사실 — 당시 `quote` 프레임 payload가 맨 호가창이고 가격이 아예 없었다 — 인데 **§16.36이 그 전제를 없앴다.** 이제 프레임은 REST 응답과 같은 `projectQuote`를 통과하며 원장이 값을 매기는 규칙(체결가 → 최우선 매도 → 최우선 매수, §16.33)으로 `price`를 말한다. 같은 payload의 호가창에서 두 번째 가격을 유도하면 러너와 원장이 종목 가격을 두고 갈라지고, 얻는 것이 없다. 따라서 호가 단위 반올림 설정도 만들지 않는다. SDK의 `TickPriceSource`에는 `'stream-quote'`를 더해 두 경로(스트림/REST)를 이름으로 구분하고, `'book-mid'`는 유니온에 남기되 생산하지 않는다 — 정말 호가창밖에 없는 어댑터가 생길 때 쓸 이름이고, 이 결정 이전에 기록된 틱 시계열도 읽혀야 한다. | `quote-ticker.test.ts`, `rest-quote-feed.test.ts`("never reports a book-mid price"), `packages/strategy-sdk/src/strategy.ts` |
| 16.40 | 전략 러너 설계 §5.3 "재연결하면 … 그 지점을 `gap` 표시로 남긴다" | **재연결 자체는 갭이 아니다.** 갭은 "관측하지 못한 시간"이며, 마지막으로 그 종목을 **본** 시각에서 `gapAfterMs`를 넘겼거나 `recoveryEpoch`가 전진했을 때만 참이다(phase B가 세운 규칙 그대로). §5.3이 재연결을 갭이라 부른 근거는 "시세는 재생되지 않는다"인데 그것은 경과 시간에 대한 진술이고, 200 ms 만에 교체된 소켓은 phase A가 링을 통째로 버리고 `slowPeriod + 1` 틱 동안 진입을 보류할 만큼 놓치지 않았다. 같은 규칙이 첫 실행·재시작·폴 장애·재연결을 한꺼번에 덮는다. 아울러 §5.3의 "gap 보강"은 **보강이 아니라 재기준**이다 — 과거 시세 엔드포인트가 없으므로(§8.4) 소켓이 죽은 동안 시장이 지나간 가격은 누구에게도 없고, 보간은 전략이 평균 내는 계열에 지어낸 관측을 넣는 일이다. 연결이 설 때마다 도는 REST 읽기는 **수준**을 복원하지 계열을 복원하지 않으며, 그 값은 "호가가 다음에 움직일 때"가 아니라 지금 틱이 오기 시작한다는 것뿐이다(한산한 종목에서는 분 단위 차이). | `quote-ticker.test.ts`(갭 판정), `market-feed.test.ts`(재기준이 틱을 중복시키지 않음), `strategy-runner-stream.integration.test.ts`("marks the tick after an outage the market moved through as a gap") |
| 16.41 | 전략 러너 설계 §8.2 "`--record`로 남긴 NDJSON 틱", §8.1 상태 파일 집합 | **`--record` 플래그는 없고, §8.1이 열거하지 않은 `ticks.ndjson`이 생긴다.** 러너 진입점은 인자를 받지 않고 환경으로만 설정되므로(`main.ts`) 녹화는 `BOT_TICK_LOG`으로 켠다 — 산출물은 §8.2가 말하는 그 NDJSON이고, 같은 `AppendLog`로 쓰고 같은 리더로 검증한다. 기본은 **꺼짐**이다: 로그는 회전하지 않고(§8.1의 미해결 항목 그대로), §8.4대로 기록한 틱이 유일한 백테스트 입력이므로 녹화는 재생하고 싶은 기간이 오기 **전에** 내리는 결정이다. `AppendLog.append`는 반쯤 쓰인 레코드 뒤로 영구히 거부하는데(스플라이스 방지), 틱 로그는 멱등성 논증의 한 단계가 아니라 나중 백테스트의 입력이므로 레코더는 그 거부를 **한 번** 보고하고 기록만 멈춘다 — 연구 산출물을 못 쓴다고 거래를 멈추는 것은 틀린 방향의 실패다. | `tick-log.test.ts`, `runner/supervisor.test.ts` |
| 16.42 | 전략 러너 설계 §8.2 "체결은 `SimulatedExchange`(지정가 도달 시 체결, 시장가는 반대편 최우선호가)" | 그 두 규칙에 **현금·포지션·수수료·미체결 주문**을 더한다 — 전략이 돈을 벌었다고만 하고 그 돈이 있었는지는 말하지 않는 보고서는 보고서가 아니다. 그리고 `STOP`·`TAKE_PROFIT`·`OCO`는 조용히 체결되지 않는 대신 **거절한다**: 트리거를 모델링하지 않으면서 영영 체결되지 않게 두면, 보호 주문이 아무것도 하지 않은 전략을 보호가 필요 없던 전략으로 보고하게 되고 그것이 백테스트가 할 수 있는 가장 위험한 말이다. 시장성 지정가는 더 나은 최우선호가가 아니라 **자기 지정가**에 체결된다 — 스냅샷은 호가가 얼마였는지 말할 뿐 얼마나 있었는지는 말하지 않으므로, 가격 개선은 하니스가 지어낸 수가 된다. 깊이도 큐도 없으므로 부분 체결이 없고 대기 순번도 없다(둘 다 전략을 실제보다 좋게 보이게 하는 방향이라 모듈 문서와 README 에 명시한다). 대기 매수의 현금 예약은 **체결이 치를 비용 전체**(명목 + 수수료)다 — 대기 지정가는 자기 지정가로 자기 수량만큼 체결되므로 수수료가 접수 시점에 완전히 결정되고, 즉시 체결 경로가 이미 검사하는 `notional + fee` 와 같은 수가 된다. 명목만 예약했던 초기 구현은 체결 때 아무것도 잡아두지 않은 수수료를 내서 지갑을 `-690` 으로 만들었고(코드를 **실행한** 리뷰어가 찾았다), 거부도 표시도 없었다. 지금은 접수에서 거부되고, 그와 별개로 `#settle` 이 잔고를 0 아래로 내리는 정산을 fail closed 로 막는다 — 중단된 재생은 메시지지만 음수 지갑은 아무도 틀린 줄 모르는 보고서다. 수수료는 §1 13행대로 계획의 것이지 원장의 것이 아니며 §8.3대로 보고서 머리가 그것을 말한다. | `simulated-exchange.test.ts`, `engine.test.ts` |
| 16.43 | 전략 러너 설계 §6.3 "장중 한정 — `GET /api/v1/markets/:m/session`의 `phase === 'REGULAR'`" | 재생에서는 **계획이 `marketPhase`를 명시**하고 그 값이 모든 시장·전 구간에 적용된다. 기록된 틱은 캘린더를 싣지 않으므로 그 엔드포인트가 무엇을 답했을지 재생이 알 방법이 없고, 지어내면 장외 거래를 열거나 장중 거래를 닫는다. **게이트 자체는 같은 것을 쓴다** — §8.2가 요구하는 것이 그것이므로 `RiskGateOptions.sessions`·`.state`의 타입을 `MarketSessionCache`·`StateStore`에서 그 둘이 이미 만족하는 `MarketPhaseSource`·`DailyNotionalSource` 인터페이스로 좁혔다. `#private` 필드를 가진 클래스는 구조적으로 대체할 수 없어 이것이 없으면 재생이 두 번째 게이트를 갖게 되는데, 그것이 §8.2가 금지하는 유일한 것이다. 게이트의 동작은 바뀌지 않는다. §16.46 이 들어온 뒤로 이 인터페이스는 §6.4 의 실현손익 질문(`realizedPnlOn`·`consecutiveLosses`)도 함께 나르며, 재생에서 그 답을 주는 것은 `SimulatedExchange` 자신이다 — 재생의 체결이 재생이 가진 유일한 실현손익이므로 이것은 대역이 아니라 진짜 출처다. 백테스트의 일일 명목 예산도 같은 질문에 같은 방식으로 답한다(모든 결정의 명목을 기록하고 질의에서 진입만 거른다) — 다만 메모리에만 있고, 재생의 예산은 재생의 속성이기 때문이다(§1 7행은 **라이브** 카운터에 대한 것이라 해당하지 않는다). | `engine.test.ts`, `risk/risk-gate.test.ts` |
| 16.44 | 전략 러너 설계 §5.3 "구독 상한(4)을 넘는 설정은 기동 시 거부한다" | **백테스트 계획에는 적용하지 않는다.** 재생은 아무것도 구독하지 않으며, 기록해 둔 5종목 시리즈를 재생하는 것은 라이브 러너가 그 설정을 거부하더라도 정당한 연구 질문이다. 계획의 나머지는 러너 자신의 `readStrategies`·`readRiskLimits`·`assertOneStrategyPerInstrument`를 그대로 지나므로, 계획이 러너가 거부할 봇을 기술할 수는 없다 — 상한 하나만 예외이고 그것이 여기 적힌 이유다. 라이브 경로의 상한은 그대로다. | `backtest/plan.test.ts`(같은 5종목이 계획에서는 통과하고 `loadRunnerConfig`에서는 거부되는 것을 한 테스트가 양쪽으로 고정한다) |
| 16.45 | §16.37 의 `fillRecord()` 단일 빌더, 그리고 시장→결제통화 유도 | §16.37 이 "이벤트와 엔드포인트는 `fillRecord()` 하나를 공유한다"를 세웠지만, **포트폴리오 스냅샷의 `activeOrders[].fills` 는 같은 파일 안에서 손으로 조립된 채 남았다** — `{id,symbol,quantity,price,fee,recoveryFill}` 여섯 필드로, 통화가 아예 없었다. 빌더 바로 위 주석이 경고하던 그 표류를 같은 리포지토리의 다른 함수가 밟고 있었다. 그래서 같은 체결에 대해 `GET /api/v1/fills` 는 통화를 답하고 스냅샷은 답하지 않았고, 스냅샷만 읽는 웹의 체결 토스트와 체결 내역이 금액을 맨숫자로 그렸다(사용자 보고: "원화에는 ₩ 달러에는 $"). 두 번째 조립을 **제거**해 스냅샷도 `fillRecord()` 를 지나게 한다 — 필드를 하나 더 붙이는 것이 아니라 갈래를 없애는 것이다. 그 결과 스냅샷 fill 행이 `FillRecord` 전체로 넓어지고 `recoveryFill` 은 빌더의 이름인 `isRecoveryFill` 이 된다. 이름은 `feeCurrency` → `currency` 로 넓힌다 — 원장이 `price × quantity ± fee` 를 한 지갑에서 결제하므로 그 통화는 수수료만의 것이 아니다. 소비자는 같은 커밋에서 따라간다: 웹 체결 내역·체결 토스트, 그리고 `@moi/strategy-sdk` 의 `BrokerFill`(SDK 어휘로 번역하지 않고 서버 이름을 그대로 쓴다 — 번역이 곧 §16.32 가 말하는 그것이다. SDK 에는 서버가 실제로 뭐라고 답하는지 고정하는 테스트가 없었으므로 함께 추가한다). **시장→결제통화 유도는 원장 레이어에만 다섯 벌 있었다** — `fill-settlement.ts`·`fill-schemas.ts`(§16.37 이 추가)·`order-placement-service.ts`(모듈 private)·`fee-schedule.ts` 두 곳·`production-runtime.ts`. 전부 `market === 'KR' ? 'KRW' : 'USD'` 를 다시 쓴 것이다. `@moi/trading-core` 의 `domain-types.ts` 로 하나를 올리고 다섯을 돌린다: `Currency`·`Market` 이 거기 살고, `createFeeModel` 이 이미 `Fee model currency must match its market` 로 같은 불변식을 강제한다. 하나여야 하는 이유는 관례가 아니다 — `assertExactMoney` 는 합의 **정밀도**를 지키지 **단위**를 보지 않으므로, 서로 다른 통화를 더한 원장은 통과하는 수를 내고 조용히 틀린다. `@moi/market-data` 의 `MARKET_CURRENCIES` 는 합치지 않는다: 토스가 어떤 통화로 호가하느냐는 프로바이더 사실이고 어느 지갑에서 빼느냐는 원장 사실이며, 오늘 같은 답을 낼 뿐 같아야 할 이유가 없다(양쪽에 그 구분을 주석으로 남겼다). 웹은 여전히 유도하지 않는다 — `apps/web` 은 `trading-core` 에 의존하지 않고, 통화는 fill 행이 말해준 것만 쓰며 모르는 값이면 기호 없이 그린다(`lib/currency.ts` 의 기존 규칙). | `market-currency.test.ts`(접기 전 기대값을 그대로 유지 — 그것이 동작 불변의 증거이며, fee model 과 fill record 라는 관측 이음매로 확인), `fee-schedule.integration.test.ts`(이미 게시된 스케줄 행이 그대로 수용되는지 — 값 비교가 아니라 릴리스가 막히지 않는지를 컨테이너로; 저장 currency 가 다르면 거부되는 부정 테스트가 짝), `paper-broker.decode.test.ts`(서버가 적는 이름으로 쓴 픽스처), `fill-announcement.test.ts`·`fill-toasts.test.tsx`(통화 기호와 모르는 통화의 맨숫자), `portfolio-routes.integration.test.ts`·`fill-history.integration.test.ts`, `fill-toast.spec.ts`·`order-lifecycle.spec.ts`(e2e 48/48) |
| 16.46 | 전략 러너 설계 §6.4 "이벤트 처리는 커서 전진과 같은 트랜잭션에서 기록한다(중복 발주·누락 방지)" — 설명하지 않는 경우: **설명 불가능한 fill** | **회계할 수 없는 fill 은 건너뛰지 않고 멈춘다(fail closed, AGENTS.md 규칙 6).** 세 모양이 "체결이 일어났는데 러너가 어느 체결인지 말할 수 없다"에 해당한다 — (a) 자기를 싣고 온 이벤트와 **다른 `accountSequence`** 를 적은 fill 레코드(원장이 두 이벤트에서 payload 를 조립했다는 뜻), (b) 읽을 수 없는 레코드, (c) `fills` 배열이 아예 없는 fill 이벤트. 초안은 셋 다 `continue` 로 건너뛰고 **이벤트는 커밋했다**. 커서가 wedge 되지 않으니 안전해 보이지만 방향이 반대다 — 커서가 그 fill 을 지나가고, 크래시와 달리 **재생할 것이 없다**(러너가 스스로 지나치기로 한 것이므로). §11 이 배제하려는 "체결 누락" 을 러너가 자기 판단으로 만들어내는 유일한 경로였고, 저장소가 다른 모든 곳에서 지키는 fail-closed 와 이 지점만 달랐다. 이제 셋 다 `INVARIANT_VIOLATION` 을 던진다: 커서가 그 자리에 머물고, `StreamClient` 가 throw 를 담아 보고하므로 프로세스는 살아 있으며 재연결마다 같은 이벤트를 재생하며 시끄럽게 실패한다. 원장이 고쳐지면 움직이지 않은 커서에서의 재생이 그 fill 을 **잃은 것이 아니라 전달**한다. wedge 가 곧 목적이다 — 계약이 허용하지 않는 것을 원장이 발행했다는 증거가 있는 상태에서 계속 거래하는 것보다 멈추는 편이 낫다. 다만 wedge 가 멈추는 것은 **체결 처리**이지 틱 기반 거래가 아니다(러너 전체 정지는 §7.2 배리어, phase D). 세 producer 확인: `fill-persistence.ts` 와 `trigger-persistence.ts`(STOP·TAKE_PROFIT) 둘 다 `fillRecord()` 로 `fills` 를 싣으므로(#43) (c) 로 정상 이벤트가 wedge 되지 않는다. | `fill-processor.test.ts` "stops on a fill record that names a different account sequence" · "stops on a malformed fill record, taking the rest of the event with it" · "stops when a fill event carries no fill records at all" (세 건 모두 RED 확인 후 구현) |
| 16.47 | §16.45 "`apps/web` 은 `trading-core` 에 의존하지 않고", 그리고 §16.37 이 목적으로 든 "실현손익" | **웹이 `@moi/trading-core` 에 의존하게 된다 — 판매 수익(실현손익)을 위해.** 원장은 실현손익 컬럼을 갖지 않는다(`fill-settlement.ts` 는 매 정산에 `realizedPnl: '0'` 을 넣는다). 대신 포트폴리오 스냅샷의 `activeOrders[].fills` 가 `fillRecord()` 전체(§16.45)를 싣고 activeOrders 는 전 주문(§16.32)이므로, 웹은 이미 세션의 모든 체결을 갖고 있고 스트림으로 갱신받는다. `features/portfolio/realized-pnl.ts` 가 그 행들을 `(market, symbol)` 별로 — 원장이 포지션을 세는 키(`unique (session_id, market_code, symbol)`)와 같게, 종목만으로는 아니다 — `fillSequence` 순(§16.37: 세션 안에서 할당 순서 = 커밋 순서)으로 `applyFillToPosition` 에 접으면 `PositionCost.realizedPnl` 이 나온다. `GET /api/v1/fills` 재조회는 하지 않는다 — 스냅샷이 같은 행을 이미 들고 있어 두 번째 출처가 될 뿐이다. 의존을 들인 이유는 규칙 5 다: 금전 산술을 웹에 두 번째로 적지 않는다(`trading-core` 는 `decimal.js` 만 쓰는 브라우저 안전 패키지고 turbo 의 `test`·`typecheck` 는 `^build` 에 의존한다). §16.45 의 나머지는 그대로다 — 통화 유도는 하지 않고 fill 행의 `currency` 를 나르며, 한 포지션의 행들이 통화를 달리 말하면 그 포지션을 **계산 불가**로 표시한다. 접을 수 없는 포지션(필드 누락·보유 초과 매도·통화 불일치·순서 키 없음)은 사유와 함께 `unavailable` 로 분리되고 나머지 포지션과 통화별 합계는 그대로 나온다 — 렌더 경로라 throw 하지 않는다(#73). **#81 의 주의**: 원장의 `average_cost` 는 반올림된 평균에서 재구성되고 이 접기는 `totalCost` 를 정확히 누적하므로 둘은 비트 단위로 일치하지 않는다(40건에 4.48e-8); 표시는 소수 두 자리 캡이라 드러나지 않지만 구성적 보장으로 가정하지 않는다. | `realized-pnl.test.ts`(수수료 넷팅·fillSequence 순서·부분 매도·시장별 동일 심볼 분리·사유 있는 계산 불가·통화별 합계), `positions-table.test.tsx`(열·부호·계산 불가 셀·market+symbol 조회), `realized-pnl-summary.test.tsx`, `currency.test.ts`(`withSignedCurrency`), `order-lifecycle.spec.ts` "reports the profit a round trip realized"(e2e) |
| 16.48 | 아키텍처 설계 §9.2 "kill switch는 resting order를 자동 취소하지 않는다. cancel-all은 별도의 명시적·멱등 운영 명령이다"; 전략 러너 설계 §6 "킬 스위치가 걸리면 (1) 모든 미체결 주문 취소 시도 … 해제는 사람이 파일에서 지우고 재시작", §7.2 "10회 실패 시 킬 스위치"; §16.46 "러너 전체 정지는 §7.2 배리어, phase D" | **봇의 킬 스위치는 자기 미체결을 스스로 취소한다** — 아키텍처 §9.2 는 원장의 포스처이고(온콜이 있는 서비스는 사람이 cancel-all 을 고른다), 온콜 없는 컨테이너 속 봇은 노출을 스스로 줄이는 쪽이 fail-closed 다. 취소는 `/admin/cancel-all` 이 아니라 세션 범위의 `cancelOrder` 를 **게이트웨이 경로로** 낸다(`decisionId = kill:{engagedAt}:{orderId}` — 감사 행, 재스윕 멱등, 실패한 취소는 pending cancel 로 남아 `recoverPending` 이 재시도). 포지션은 청산하지 않는다. **배리어가 잡은 결정은 `halted` 로 정산한다**(pending 으로 두면 사람이 래치를 풀고 재시작한 순간 트립 전 진입이 나간다 — 킬 스위치가 잡은 결정은 죽은 결정이다); `SubmissionOutcome` 에 `halted` 가 는다. 배리어는 `OrderGateway.submit()` 한 곳, 매 attempt 전과 **실패한 attempt 뒤에** 묻는다(백오프 중 내려온 래치가 다음 전송을 막고, 요청이 날아가 있는 동안 내려온 래치는 그 place 를 pending 대신 halted 로 정산한다); cancel 은 통과한다. 백오프는 5분(§7.2)을, 스윕의 in-flight 대기는 5초를 넘지 않는다. 래치 쓰기 실패는 던지지 않고 보고 후 cycle 마다 재시도하며 메모리 배리어는 닫힌 채다; 실행 중 파일 읽기 장애(EIO 등)는 래치가 아니라 전이 1회 `warn` 이다(파싱 불가 파일은 래치다; **시작 시** 읽기 장애는 fail closed 로 메모리 래치다). 스윕의 idle 대기 상한이 이기면 `idle()` 이 풀릴 때 한 번 더 스윕한다. 세션 재수립이 실패하는 동안 래치가 내려오면 그 place 도 halted 다. 스윕 실패 보고는 코드만 낸다. §7.2 의 "10회" 는 **결정을 가로지른 연속 실패 attempt** 로 읽는다(성공이 리셋, 거부는 세지 않음, 상수 `KILL_SWITCH_AFTER_FAILED_ATTEMPTS`). 설계 §6 의 두 손실 한도는 BUY 거부에 더해 킬 스위치로 **격상**한다(`RiskGate.lossLimitBreach()`, cycle 마다). §16.46 의 wedge 는 이제 `engage('fill-wedge')` 를 함께 부른다 — 커서는 그대로, 재생마다의 재호출은 멱등이라 조용하다. **걸린 러너는 살아서 지켜본다**(종료하면 `restart: unless-stopped` 가 재시작 루프를 만든다): 스트림·fill 처리·커서·레코더는 계속, 호스트에 틱만 주지 않고 30분마다 `warn`. 설계에 없는 추가 하나: **운영자가 `kill-switch.json` 에 `{"reason":…}` 을 써서 건다**(해제와 대칭, 읽을 수 없는 파일도 건다 — fail closed). 재시작은 래치를 보고하고 **다시 스윕한다**(첫 스윕 도중 죽었을 때 기록되지 못한 주문을 잡는 유일한 길; id 가 같아 두 번 기록되지 않는다). 시도 0회로 `halted` 된 결정은 `dailyEntryNotional` 에서 빠진다(내지 않은 주문이 예산을 깎으면 같은 날 래치를 푼 운영자가 부족해진다; 시도 뒤 halted 는 원장에 닿았을 수 있어 센다 — `SubmissionRecord.attempts`). `resume()` 은 디스크에서 읽은 래치에만 한 번 말한다(같은 실행에서 걸린 것은 `engage` 가 이미 보고했다). 설정 항목은 없다 — 임계값 셋은 상수다. 설계 문서: `2026-09-02-moi-strategy-runner-kill-switch-design.md`. | `kill-switch.test.ts`(멱등 engage·파일 우선·전이 1회 보고·운영자 파일 세 모양·idle 대기·재스캔·5패스 잔존·resume 재스윕·heartbeat), `order-gateway.test.ts` "under the kill switch barrier"(halted 정산·cancel 통과·attempt 사이 배리어·idle·10회 소진·리셋/거부 무시), `state-store.test.ts` "halted … settled, across a reopen", `risk-gate.test.ts` "lossLimitBreach", `fill-processor.test.ts` §16.46 세 모양 + `engage('fill-wedge')`, `supervisor.test.ts` "the kill switch in the runner"(운영자 파일·손실 한도·재시작 보고, 그리고 배선 고정 4건: 래치 아래 pending place 가 halted / 503 연속 → submission-failures / 스트림 wedge → fill-wedge / heartbeat), `strategy-runner-kill-switch.integration.test.ts`(원장에서 두 미체결이 CANCELLED, 이후·재시작 뒤 주문 0) |
| 16.49 | 전략 러너 설계 §3 "`apps/strategy-runner`는 `@moi/strategy-sdk`와 `@moi/trading-core`에만 의존한다"; §11 D "Discord 리포터 + compose/systemd 통합 — 호스트에서 임베드 수신" | **`@moi/strategy-reporter` 에도 의존한다.** §7.4 의 Discord 리포터는 그 패키지에 있고(#82), 러너가 그것을 조립하지 않으면 §11 D 는 닫히지 않는다. 경계의 목적은 원장 내부(`@moi/paper-api`)와 프로바이더 어댑터(`@moi/market-data`)를 결정 경로에서 떼어 놓는 것이고, 리포터 패키지는 둘 다 닿지 않는 순수 텍스트·HTTP 어댑터(자체 의존성 0)라 금지 목록은 그대로다 — `package-surface.test.ts` 가 허용 셋과 금지 넷을 함께 고정한다. 같은 이유로 마스커도 그 패키지의 `maskOutbound` 하나로 합쳤다(#92): 두 벌이던 규칙을 각각 고치던 비용이 통일의 이유고, 마스크 토큰은 `[redacted]` 에서 `***` 로 바뀐다. 배선(`runner/reporter-wiring.ts`)은 stdout 과 Discord 를 팬아웃한다 — `docker logs` 가 운영자의 첫 시선이다 — 그리고 형식이 틀렸거나 운영 채널과 같은 `DISCORD_WEBHOOK_TRADE_URL` 은 러너도 **기동을 거부**한다(preflight 와 같은 판정; 비어 있으면 stdout 만). 종료 시 Discord 큐를 flush 한다. **bot 이 스택에 붙는 방법은 설계에 없던 결정이다** — `/etc/moi/moi.env` 의 `COMPOSE_PROFILES=bot`: systemd 와 deploy.sh 가 이미 읽는 파일이라 pull/up/stop 이 저절로 포함하고, deploy verify 는 프로필이 켜져 있으면 bot 컨테이너의 running 을 요구한다(설정 거부의 재시작 루프를 `restart: unless-stopped` 뒤에 숨기지 않는다). 이미지는 paper-api 와 같은 4단계 비-root 이고 `/var/lib/moi-bot` 을 `node` 소유로 만든다 — 이름 있는 볼륨이 그 소유권을 물려받아야 read-only 루트에서 상태를 쓴다. Dockerfile·publish 매트릭스·GHCR overlay 는 한 커밋이고 계약 체커가 셋의 관계(Dockerfile 을 가진 compose 서비스 ↔ publish 항목 ↔ overlay 이미지)를 검사한다. 운영자 예시 설정은 테스트가 실제로 로드한다(phase C 의 두 손실 한도가 빠져 있었다). | `reporter-wiring.test.ts`(stdout 전용·팬아웃+값 마스킹·형식 오류 거부·동일 채널 거부·close 가 flush), `package-surface.test.ts`, `config.test.ts` "infra/bot/runner.example.json loads", `check-deployment-contract.test.mjs`(publish 매트릭스·overlay·CMD·상태 디렉터리 소유·deploy verify 변이 5건), 이미지 자체의 빌드·비-root·상태 디렉터리 소유는 저장소 테스트가 아니라 PR #101 본문의 로컬 arm64 `docker build` 기록과 `Publish images` 의 Trivy 게이트가 증거다 |
| 16.50 | §7.5 "`publishQuote`도 `LIVE`에만 전달한다"; §7.5 U11 "클라이언트 수신 순서가 정확히 `ready` → …" | **OPENING 창의 quote 가 유실됐다.** `StreamSession.open` 은 `ready` 를 보낸 뒤 DB 에서 계정 이벤트를 재생하고, 허브는 그 뒤에야 엔트리를 LIVE 로 올린다 — 그런데 quote 는 LIVE 엔트리에만 송출됐다. 클라이언트는 `ready` 를 받은 순간부터 연결됐다고 믿고(전략 러너는 그 순간 REST 재기준까지 마친다) 그 창에 시장이 낸 시세는 어디에도 없다: 계정 이벤트는 큐에 쌓여 승격 때 흘러가지만 시세는 재생 대상이 아니므로(§7.5 의 replay 는 계정 이벤트만이다) **다음 호가 변화까지 아무 가격도 오지 않는다**. 재시작한 러너에서 재현됐다(커서가 있어 replay 가 길다 → 창이 넓다): `ready(6)` 뒤 v7·v8 두 시세가 OPENING 에 떨어져 사라지고 러너는 30초를 기다렸다 — 킬 스위치 통합 테스트(#100)가 CI 에서 실패하고 로컬 단독 반복에서 4~5회 중 1회 재현된 원인. 고침: 허브가 업그레이드 쿼리의 요청 심볼(`?quoteSymbols=`, 101 이전에 거래 가능·중복·상한 검증을 이미 지난 집합)을 엔트리에 갖고, OPENING 엔트리에도 그 심볼의 quote 를 소켓으로 바로 보낸다(`quoteFrame` 빌더 하나를 세션과 허브가 공유; 엔트리마다 두 경로 중 정확히 하나). LIVE 뒤에는 이전처럼 세션의 구독 집합이 판정한다. **계약 변화 하나**: 엔트리는 `open` 의 첫 await 전에 등록되므로 `quote` 가 `ready` 보다 먼저 올 수 있다 — `ready` 는 더 이상 첫 프레임이 아니다. 두 클라이언트 모두 견딘다(러너 `#quote` 는 ready 에 게이트하지 않고 `QuoteTicker` 가 버전으로 중복을 거른다; 웹 `useQuoteStream` 은 심볼만 본다). `ready` 를 승격 뒤로 옮기는 대안은 재생 이벤트가 `ready` 앞에 오게 되어 heartbeat 무장(U11·W3)이 깨지므로 택하지 않았다. | `stream-hub.test.ts` "forwards quotes for requested symbols to an OPENING entry"(요청 심볼만) · "forwards nothing to an OPENING entry that asked for no symbols" · U13(LIVE 엔트리가 같은 집합을 들고도 quote 를 한 번만 받는다 — 두 경로 동시 송출 뮤턴트가 문다); `stream-upgrade.test.ts` "a quote published during replay reaches the client before the replayed events"(배선: 요청 심볼만, LIVE 뒤 단일 팬아웃) |
| 16.51 | 전략 러너 설계 §3 "`onTick`은 **동기·순수**하다"(`onFill` 에는 같은 문장이 없었다); §5.3 "재시작 후 같은 결정을 다시 제출해도 원장은 주문을 한 번만 만든다. 키는 결정이 저장된 **뒤에** 계산되므로 크래시 시점과 무관하게 재현된다" | **`onFill` 도 순수해야 하고, 러너는 그것을 검사한다(#88).** fill 경로의 결정 id 는 `fill:{accountSequence}:{strategy}:{index}` 로 재생 때 다시 계산되므로 §5.3 의 재현성은 `onFill` 이 같은 fill 에 같은 결정을 내는 데 달려 있다. 순수하지 않은 전략은 로그가 다른 결정을 쥔 id 로 새 결정을 낸다 — 기록된 것을 보내면 전략이 더는 원치 않는 주문이 나가고, 새 것을 보내면 원장이 같은 키의 다른 해시를 `IDEMPOTENCY_CONFLICT` 로 거부한다(`toOutcome`). 그래서 (1) SDK 가 `onFill` 에 `onTick` 과 같은 순수성 계약을 문서로 건다(biome 의 전역 금지는 내장 전략만 덮는다 — 외부 전략은 계약을 스스로 진다); (2) `OrderGateway.record()` 는 이미 기록된 id 를 만나면 **기록된 결정**을 돌려준다(`StateStore.decision(id)`) — 제출되는 것은 언제나 로그가 쥔 결정이다; (3) `FillProcessor` 는 이벤트의 fill 전부에 대한 호스트별 답을 먼저 모아(id 의 index 는 이벤트 안 그 호스트의 결정을 fill 을 가로질러 이어 센다 — 한 주문이 호가 두 레벨을 먹으면 한 `ORDER_FILLED` 에 fill 이 둘이다) **아무것도 기록하기 전에** 기록과 대조한다(kind·intent 필드별·orderId, 그리고 `(accountSequence, host)` 당 결정 **수** — 기록된 id 와 새 id 가 섞이거나 기록된 다음 id 가 남아 있으면 발산; 첫 실행이 결정을 하나도 내지 않은 경우는 기록이 없어 잡지 못한다) — 다르면 `INVARIANT_VIOLATION` "onFill diverged on replay" 로 §16.46 의 웨지와 같은 길을 간다: 커서 불변, `error` 한 줄, 킬 스위치 `fill-wedge` 트립(§16.48), 재생마다 같은 자리에서 다시 선다. 자동 복구는 없다: 어느 쪽 결정이 맞는지는 코드가 알 수 없고, 사람이 전략을 고쳐 재시작하면 재생이 기록과 맞아 지나간다. `reason` 은 비교하지 않는다(자유 텍스트, 주문에 영향 없음). | `fill-processor.test.ts` "wedges when the replayed onFill wants a different order" · "… answers nothing where it had placed" · "… answers more than it had"(기록 전 판정: pending 은 원래 하나뿐) · "… when a noop replaces a recorded place" · "numbers one strategy’s decisions across every fill in the event" · "replays a two-fill event against the same numbering"(변이: 필드 비교 제거 → 2건, 수 비교 제거 → 2건이 문다); `order-gateway.test.ts` "returns the recorded decision rather than the one offered now"; `state-store.test.ts` "StateStore decision lookup" 3건 |
| 16.52 | 전략 러너 설계 §5.4 "끊기면 지수 백오프 재연결"; §7.2 "`5xx`, 네트워크: 백오프 재시도. 3회 실패 시 `warn`, 10회 실패 시 킬 스위치"; `ReconnectPolicy` 의 창(120초 안 5회 → hold 밴드) | **연결 성공은 `ready` 가 아니라 `ready` 뒤 하트비트 인터벌 하나를 살아남은 시점에 기록한다(#89).** 이전에는 `ready` 프레임이 곧 `recordSuccess()` 였다. 업그레이드를 받고 `ready` 를 답한 뒤 곧바로 닫는 서버(배포 중 재시작 루프, 리버스 프록시 idle 종료 등)는 매 시도마다 정책을 리셋해 실패 창이 5 에 닿지 못하고, 러너는 ordinary 밴드의 짧은 지연으로 무한히 재접속했다 — hold 밴드와 "has failed repeatedly" 경고가 존재하는 이유가 바로 그 폭풍인데 그 폭풍만 걸러내지 못했다. 고침: `ready` 에서 `STABLE_AFTER_INTERVALS`(=1) × 하트비트 인터벌 짜리 안정 타이머를 무장하고 그것이 만료돼야 `recordSuccess()`. 그 전에 소켓이 끊기면 타이머는 `#detach` 에서 취소되고 그 연결은 실패 하나로만 남는다. 정상 연결의 체감 차이는 없다(30초 뒤 리셋 — 다음 실패는 어차피 그 뒤다). `onReady`·REST 재기준·틱 소비는 전과 같이 `ready` 즉시다: 미뤄진 것은 정책 리셋뿐이다. 광고된 인터벌은 `MIN_HEARTBEAT_MS`(1초)·`MAX_HEARTBEAT_MS`(5분) 사이로 clamp 한다 — `1` 을 광고하는 서버(또는 프레임을 고쳐 쓰는 프록시)가 안정 창을 형식으로 만들지 못하게, 그리고 Node 가 2^31−1 ms 초과 `setTimeout` 을 1 ms 로 clamp 하므로 거대한 값이 같은 결과를 내지 못하게. | `stream-client.test.ts` "treats ready-then-close as a failure, so a flap storm reaches the hold band" · "does not record success for a connection that was dropped before the interval" · "resets the backoff once a ready connection has held for a heartbeat interval"(변이: `ready` 즉시 `recordSuccess()` 로 되돌리면 3건이 문다) · "floors an absurd advertised heartbeat interval" · "caps an absurdly large advertised heartbeat interval" |
| 16.53 | §16.30 배포 스크립트(`deploy.sh` 시작/완료/실패 알림, `/run/moi-deploy.lock` 마커) — 운영 추가; §16.28 (1) "요청 ref를 fetch 후 detached checkout"; §16.49 "bot 이미지는 paper-api 와 같은 publish 매트릭스·GHCR overlay" | **참조 호스트 배포를 세 겹으로 닫는다(#44 · #28 · #83). 런타임 동작 변경 없음.** (1) **직렬화(#44)**: `deploy_begin` 이 fetch 전에 `/run/moi-deploy.mutex` 에 비차단 배타 `flock` 을 잡고(디스크립터 9, exit 트랩까지 보유) 두 번째 호출은 exit 75 로 체크아웃·마커·타이머·알림 어디에도 손대지 않는다. `/run/moi-deploy.lock` 은 상호 배제가 아니라 상태 타이머를 침묵시키는 **마커**일 뿐이라 별도 뮤텍스가 필요했다; 뮤텍스를 잡은 프로세스만 마커를 지운다(`DEPLOY_OWNS_MUTEX`). `flock` 부재는 exit 69 로 fail-closed(`bootstrap.sh` 가 `util-linux` 를 설치). (2) **re-exec(#28)**: fetch·checkout 뒤 첫 단계 프로세스가 체크아웃된 `infra/oracle/deploy.sh` 로 `exec` 한다 — 이전엔 fetch 전 복사본이 끝까지 돌아 배포 스크립트 수정이 **다음** 배포에나 효력을 냈다. 디스크립터 9 와 마커는 `exec` 를 넘어 상속되고 교체 프로세스는 `MOI_DEPLOY_REEXEC=1` 아래서 둘을 검증해 뮤텍스를 **채택**한다(재생성·두 번째 `deploy started` 없음; 가드 위조는 exit 70). (3) **provenance(#83)**: `main` 같은 가변 태그는 선택자일 뿐이라 publish 가 실패하거나 지연되면 `deploy.sh` 는 옛 이미지를 끌어와 마이그레이션·재시작까지 마친 뒤 성공을 보고했다. publish 는 매 이미지에 `org.opencontainers.image.revision=<sha>` 레이블을 싣고(아키별 빌드 스텝; `imagetools create` 는 config 를 바꾸지 않아 레이블이 보존된다), `deploy.sh` 는 pull 뒤 **마이그레이션 전에** `compose config` 가 해석한 모든 런타임 이미지(`paper-api`·`web`, 프로필이 켜졌으면 `bot`)의 레이블을 체크아웃 SHA 와 대조하고 누락·불일치·inspect 실패는 exit 1, 이미지가 두 개(`paper-api`·`web`) 미만이면 exit 64 로 마이그레이션 전에 멈춘다. 롤백은 `MOI_IMAGE_TAG=<full sha>` 와 같은 SHA 를 `deploy.sh` 에 넘긴다 — 태그와 다른 ref 의 조합은 마이그레이션 전에 실패한다. **재시작 뒤 실행 컨테이너를 다시 대조한다**(리뷰 HIGH): systemd 는 `/etc/moi/moi.env` 만 읽으므로 명령줄에만 핀한 `MOI_IMAGE_TAG` 는 pull·검증·마이그레이션은 `<sha>` 로, 실행은 `main` 으로 가르는데, `compose ps -q` 로 얻은 `paper-api`·`web`(·`bot`) 컨테이너의 같은 레이블이 체크아웃과 다르면 `deploy_verified` 전에 exit 1; `ps -q` 는 종료된 컨테이너를 숨기므로 id 수가 서비스 수와 같아야 한다. 프로세스 치환 안의 sops/compose 실패는 빈 배열로 떨어져 fail-closed 되지만 진단 메시지가 한 단계 어긋난다(기록만). 이미지 목록은 `compose config --images` 로 얻는다 — 전체 `config` 출력은 모든 시크릿 값을 담아 셸 변수에 둘 수 없다(규칙 2). re-exec 채택은 fd 9 가 **뮤텍스 파일 그 자체**인지 perl `stat` 으로 확인하고, 다른 배포가 락을 쥔 동안 뮤텍스 파일을 새로 연 위조도 `flock -n 9` 재획득 실패로 거부한다(`sudo -E` 로 새어 든 가드는 exit 70). `exec` 는 `-x` 검사와 `shopt -s execfail` 뒤에 한다 — 실패한 `exec` 는 EXIT 트랩을 건너뛰어 마커가 남고 알림이 없었을 것이다. **레이블 없는 기존 이미지로의 롤백은 면제가 필요 없다**: 이 검사 이전 ref 로 롤백하면 (2) 의 re-exec 가 **그 ref 의 옛 deploy.sh** 를 실행하고 거기엔 검증이 없다(fetch 두 번, `deploy started` 두 번). 검사를 가진 체크아웃이 가리킬 수 있는 sha 태그 이미지는 전부 빌드 시 레이블을 받았으므로, 누락은 손으로 밀어 넣은 이미지뿐이라 우회 스위치를 두지 않는다(리뷰 레인 1 이 제안한 `MOI_DEPLOY_ALLOW_UNLABELED` 는 레인 2 가 도달 불가·약화만 남음을 보여 제거). **기록만 하는 한계**: `flock -u 9` 는 중단된 배포의 자식(`sops → compose pull`)이 fd 9 를 물고 있어도 락을 풀어 다음 배포와 겹칠 수 있다 — `-u` 없이는 고아 자식이 락을 영구 보유하므로 덜 나쁜 쪽을 택했다. | `infra/oracle/status-check.test.mjs` deploy-lib: 뮤텍스 거부(실 프로세스 보유·본문 미실행·마커 보존)·`flock` 부재 69·re-exec 1회(옛 본문 미재개·알림 단일)·가드 위조 70·revision 일치/불일치/레이블 없음/bot stale/이미지 부족 64·실행 컨테이너 불일치/부족/일치·fd 9 가 다른 파일 70·락 보유 중 뮤텍스 파일을 새로 연 위조 70·실행 불가 스크립트로의 re-exec 가 트랩을 거쳐 exit 1(마커 제거·실패 알림)(flock 스텁은 perl 로 상속 fd 에 **실제** 락을 건다 — 프로세스별 뮤텍스 파일 뮤턴트가 죽는다; docker 스텁은 `--format` 이 레이블 키를 읽지 않으면 거부); `scripts/check-deployment-contract.mjs` "reference deploy re-executes the checked-out script"·"reference deploy verifies pulled image revisions"(`config --format json` 금지 포함)·"reference deploy verifies the running containers before reporting success"·"published runtime images carry source revision"(deploy-lib 의 `REVISION_LABEL` 과 키 동일) — 검사 전 주석 줄을 벗겨 주석 처리 뮤턴트가 통과하지 못한다 + `check-deployment-contract.test.mjs` 변이 6건(re-exec 배선 제거·레이블 제거·검증 제거·주석 처리·컨테이너 검증 제거·레이블 키 오타) |
| 16.54 | §10.2-4 "P1에 `SIGTERM` → 200 ms 내 P1 trading `reasons ⊇ ['CANCEL_ONLY','DRAINING']`, `/health/ready` 503(`draining:true`), `/health/live` 200; P1으로 보낸 신규 비즈니스 요청 → 503 `NOT_READY`"; §10.2-11 "종료 코드 0, `SIGTERM`부터 exit까지 ≤ 3 s" | **드릴 4단계는 admitted 요청 하나를 P1 안에 붙잡아 둔 채 관측한다(#65).** 원문의 단언은 그대로지만 관측 방법이 바뀐다: outbox 가 비어 있고 in-flight 요청이 없는 P1 은 `SIGTERM` 뒤 **DRAINING→STOPPED 20~35 ms, exit ~60 ms** 다(CI 증거 12회 실측, 통과·실패 무관). HTTP 폴링(50 ms)으로 그 창을 잡으려던 4단계는 러너가 한가할 때만 통과했다 — 부하 시 첫 폴이 상태 전이 직전에 닿아 200 을 받고 다음 폴은 종료된 프로세스를 만난다(`timed out waiting for P1 DRAINING` 2회, `fetch failed`/`ECONNRESET` 3회 = 3일간 드릴 실패 7건 중 5건; 나머지 2건은 아래). 고침: 하네스가 **두 번째 익명 세션**의 `anonymous_sessions` 행을 별도 연결에서 `for update` 로 잠근 뒤 그 세션으로 존재하지 않는 주문의 취소를 P1 에 보낸다 — 요청은 admitted 되어 `LEDGER_LOCK_ORDER` 첫 단계(세션 락)에서 막히고(`pg_stat_activity` 로 확인 뒤 `SIGTERM`), §6.6-3 의 in-flight drain 이 P1 을 `DRAINING` 에 붙잡는다. 4단계 단언(trading·ready·live·P1 거부 503·P2 취소 200·P2 신규 주문 409)은 그 안에서 이뤄지고 마지막에 P1 이 여전히 `DRAINING` 임을 한 번 더 단언한 뒤 락을 풀면 요청은 `INVALID_ORDER` 400 으로 끝나고(원장 무접촉) P1 은 정상 종료한다. 두 번째 세션인 이유: 같은 세션이면 P2 의 LIMIT 취소도 같은 락에 막힌다. 붙잡는 창은 `SHUTDOWN_DRAIN_DEADLINE_MS`(10 s) 안에서만 결정적이다 — 넘기면 §6.6-3 이 포기하고 막힌 트랜잭션이 풀 클라이언트를 쥔 채 `db.destroy` 가 예산을 태워 exit 1, 즉 11단계 실패와 같은 지문이 되므로 락을 풀기 전에 경과(`step4HeldMs`)가 5 s 미만임을 이름 붙여 단언한다(로컬 수십 ms). 그 경우에도 P1 로그가 원인을 말하도록 §6.6-3 의 두 drain 이 deadline 에서 포기하면 `shutdown.inflight_drain_timed_out {http, uow}` 를 남긴다(`RequestAdmissionGate.drain` 은 남은 수를 반환). 5단계의 `exit.ms < 15 s` 는 이제 "붙잡은 창 + 종료" 를 재므로 P1 종료 지연의 상한이 아니다(§10.2-11 의 3 s 는 P3 에서 따로 잰다). 스펙의 "200 ms 내" 는 유지되며 드릴 증거에 `step4DrainingObservedMs` 를 **기록만 하고 단언하지 않는다**(관측 지연은 신호 전달 + fetch 한 번이라 부하 시 200 ms 를 넘길 수 있고, 붙잡힌 뒤라 늦게 봐도 같은 상태다; 로컬 1 ms; 신호 뒤 500 ms 지연을 넣은 실험에서도 502 ms 에 관측·통과 — 옛 설계면 그 조건에서 100% 실패). **§10.2-11 의 exit 1 은 별건**: 실패 2건의 P3 로그는 `shutdown.step_timed_out step=db.destroy "exceeded 9991ms"` — 상태 전이는 5 ms 안에 끝났고 `pool.end()` 가 예산(`SHUTDOWN_DRAIN_DEADLINE_MS`=10 s) 을 다 썼다. 풀 상수 `CONNECTION_TIMEOUT_MS`=10 s 와 일치하나 로그만으로 원인(반납되지 않은 클라이언트 vs 연결 중 클라이언트)을 가를 수 없어, 그 단계의 timeout/실패 로그에 **풀 카운터(`total`/`idle`/`waiting`)** 를 싣는다 — 다음 발생이 스스로 증거가 되도록. 카운터가 말하는 것은 나가 있던 클라이언트 수(`total - idle`)와 큐에 든 호출자 수(`waiting`) 까지다 — pg-pool 은 연결 중인 클라이언트도 `_clients` 에 바로 넣고 `waiting` 은 풀이 가득 찼거나 idle 이 있을 때만 늘어나므로, **미반납과 연결 중은 이 카운터로 갈리지 않는다**(둘 다 `total - idle ≥ 1, waiting 0`). 가르는 단서는 시간이다: 연결 중 클라이언트는 `CONNECTION_TIMEOUT_MS`(10 s) 에 스스로 끝나고 미반납은 끝나지 않는다. 테스트(A5b)는 미반납 분기를 재현한다; 연결 중 분기는 살아 있는 PostgreSQL 앞에서 결정적으로 만들 수 없어 로그 필드 계약으로만 남긴다. 테스트 시임으로 `ProductionRuntimeOptions.database` 를 두었다(`main.ts` 는 넘기지 않는다). 함께: paper-api vitest `hookTimeout` 120 s(`Hook timed out in 10000ms` 1건 — 컨테이너 기동은 한가할 때 2~4 s, Docker 가 바쁠 때 10 s 를 넘긴다; 상한일 뿐 대기가 아니다). **잔존**: testcontainers 12.1.0(최신) 의 포트 바인딩 대기 10 s 는 `withStartupTimeout` 과 무관하게 하드코딩(`inspectContainerUntilPortsExposed`) — 1건, 설정 불가. #65 선택지 3(기동 실패와 단언 실패의 종료 코드 분리)은 근인이 인프라가 아니라 테스트 경쟁으로 밝혀져 채택하지 않는다. | `leader-handoff.drill.integration.test.ts` 4단계(로컬 3회 연속 + 500 ms 지연 실험 통과, `step4Held`=400 `INVALID_ORDER`), `two-process-harness.ts` `holdSession`/`releaseSession`/`backendsBlockedOn`; `production-runtime.integration.test.ts` "A5b: a pool client nobody released pins db.destroy, and the forced stop reports the pool counters"(체크아웃 클라이언트 보유 → `forced:true`, `pool.total ≥ 1, idle 0, waiting 0`) |
| 16.55 | 전략 러너 설계 §5.4 "1차: `GET /api/v1/stream` WebSocket. 세션 쿠키로 인증한다"; §7.2 "`5xx`, 네트워크: 백오프 재시도. 3회 실패 시 `warn`, 10회 실패 시 킬 스위치"; `ReconnectPolicy` 의 hold 밴드(§16.52) | **러너는 기동 시 paper-api 가 `SERVING` 이 될 때까지 기다린 뒤에 첫 WS 연결을 연다(#112).** 첫 실전 활성화(2026-09-04, `95669f9`)에서 배포가 봇과 API 를 함께 재시작했고 API 는 RESTORING→ACQUIRING_LEASES→RECOVERING 에 ~20 s 를 쓰는데, 봇은 그 창에 업그레이드를 5회 시도해 전부 503 `NOT_READY` 를 받았다 — 소켓은 101 에 못 미친 상태 코드를 노출하지 않아(§16 stream-client 주석) 러너에겐 다른 거절과 구별되지 않는 실패 5회였고, 정상 경로가 배포마다 `stream errored`/`closed` 경고 5쌍 + hold 밴드 `error` 임베드 1개를 트레이드 채널에 냈다. 고침: `RunnerSupervisor.start()` 가 세션 수립 전에 `GET /health/market-data`(공개, 비인증)를 1 s 간격으로 폴링해 `runtime === 'SERVING'` 을 기다린다. 첫 관측이 SERVING 이면 조용하고, 아니면 `info` 한 줄("not serving yet; waiting")과 도달 시 `info`(`waitedMs`). **상한 120 s 는 대기의 천장일 뿐 거부가 아니다**: 넘기면 `warn` 한 줄 뒤 이전과 똑같이 연결을 시도하고 §5.4/§7.2 의 재연결 정책이 이어받는다 — 느린 API 가 봇을 재시작 루프에 넣어 배포 verify 를 깨우지 않게. 마감은 프로브 사이에서 검사하므로 실제 대기는 최대 "120 s + 요청 타임아웃 한 번(10 s)" 이고 `waitedMs` 는 그 값을 그대로 적는다. 응답 없음은 `runtime=unreachable`, 응답은 있으나 `runtime` 이 없으면 `http <status>` 로 적는다. 대기 중 `stop()`(SIGTERM)이 오면 다음 프로브 전에 빠져나오며 세션도 스트림도 만들지 않고 `run()` 은 즉시 반환한다 — 이전엔 그 창이 ms 단위라 없던 경로가 120 s 로 넓어져 필요해졌다. 게이트를 REST 에 둔 이유: WS 층에서는 503 을 식별할 수 없고, compose `depends_on: service_healthy` 는 `/health/ready`(db+audit) 라 `ACQUIRING_LEASES`/`RECOVERING` 에서도 200 이다. **함께**: 소켓 `error` 이벤트의 설명이 비어 있던 버그 — Node 내장 `WebSocket` 의 `ErrorEvent.message` 는 `undefined` 가 아니라 `''` 라 `?? 'unknown'` 이 걸리지 않았다. Node 24.19(undici 7)에서 실측하면 `event.error` 도 메시지가 빈 `TypeError` 라 **거절된 업그레이드와 거절된 연결은 이벤트 어디에도 이유를 남기지 않는다**(close 코드 1006 이 유일한 신호). 그래서 `describeSocketError` 는 `error.message` → `message` 를 읽고, 둘 다 비면 `not exposed by the runtime WebSocket; see the close code` 라고 **말한다** — 빈칸 대신. 원인이 정말 필요한 순간은 기동 창인데, 그 창은 위의 SERVING 대기가 없앤다. | `supervisor.test.ts` "startup waits for the API to serve"(RECOVERING·503·RECOVERING·SERVING 순서 뒤 세션 생성, `sleep` 3회·`info` 두 줄; 즉시 SERVING 이면 프로브 1회·무음; 영구 **무응답**이면 121회 프로브 뒤 `warn` 한 줄(`runtime=unreachable`)과 세션 생성); `stream-client.test.ts` "names the socket error, or says the runtime exposed none, never a blank"(원인 있는 `error`·`ws` 형태·Node 24.19 형태·빈 이벤트); `supervisor.test.ts` "names an answer without a runtime by its status, and keeps waiting through a missing response"(503 무본문 → `runtime=http 503` 으로 첫 announce, 응답 없음은 대기 지속), "stops waiting when stop() arrives, and creates no session", "does not swallow a refusal by the API client itself while waiting" |
