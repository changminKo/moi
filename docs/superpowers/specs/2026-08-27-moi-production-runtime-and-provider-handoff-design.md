...
| 16.32 | `@moi/strategy-sdk`가 페이퍼 API의 실제 응답을 해독하지 못하던 계약 드리프트 정정 (#32) | SDK가 원장 내부 타입(`OrderSnapshot`/`WalletSnapshot`/`PositionSnapshot`)을 와이어 페이로드로 재사용해, 응답에 없는 `version`을 요구하고(성공한 주문이 `INVARIANT_VIOLATION`으로 실패) API가 보내는 `market`·`averageCost`는 버렸다. SDK 소유 와이어 타입(`BrokerOrder`/`BrokerWallet`/`BrokerPosition`/`BrokerPortfolio`)을 도입해 페이로드를 그대로 반영한다. `version`은 공개하지 않는다 — 어떤 쓰기도 클라이언트 버전을 받지 않고(동시성은 `Idempotency-Key`), 클라이언트가 관측하는 순서는 `accountSequence`다. 함께 정정: 조건부·OCO 주문 와이어(SDK `triggerPrice` → API `stopPrice`, 평면 OCO → 2-leg `legs`), 부재의 표기(API는 `null`, 이전 디코더는 `undefined`만 허용해 모든 포트폴리오 조회가 실패), 401 분류(`ACCOUNT_READ_ONLY` → `SESSION_EXPIRED`; 후자를 `DomainErrorCode`에 추가 — 공개 에러 계약에는 처음부터 있었다), `PaperBrokerPath` 주석의 과장(경로만 제한하며 호스트는 트랜스포트가 정한다). API 측 변경은 하나: `/api/v1/portfolio`가 `sessionId`를 반환한다(클라이언트가 자기 세션의 계좌인지 확인할 수 있어야 하고, 값은 호출자가 이미 가진 것이다). | `apps/paper-api/src/runtime/paper-broker-contract.integration.test.ts` — 실제 런타임·실제 원장을 상대로 `PaperBroker`가 MARKET/LIMIT/STOP/TAKE_PROFIT/OCO 배치·취소·포트폴리오·멱등 재생·401을 왕복한다. 페이크 트랜스포트끼리만 합의하던 구조가 드리프트의 원인이었다. |
