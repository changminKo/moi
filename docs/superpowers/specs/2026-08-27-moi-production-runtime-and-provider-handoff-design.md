...
| 16.36 | §7.5 `quote` 프레임의 payload 모양, 그리고 §16.17 REST quote 투영 | `quote` 프레임의 payload 모양을 아무 문서도 고정하지 않은 탓에 세 계층이 서로 다른 모양을 믿었다. 프레임은 `MarketRuntime.#dispatch`가 `event.book`을 그대로 실어 **`OrderBookSnapshot`**(`{symbol,market,currency,bids,asks}`, 수량 필드는 `volume`)이었고, REST `#quote`는 `{market,symbol,price,asOf,health,recoveryEpoch,marketDataVersion}`으로 **호가를 아예 담지 않았고**, 웹의 `QuoteSnapshot`은 그 둘의 합집합인데 수량을 `size`로 적었다. 웹 훅이 `setQuote(message.payload as unknown as QuoteSnapshot)`으로 **캐스트**해 이 불일치가 컴파일에서 걸리지 않았고, `order-book.tsx`의 `Decimal.max(m, x.size)`가 `undefined`를 받아 렌더 중 동기적으로 던졌다 — `apps/web/src`에 `ErrorBoundary`가 없어 호가 패널이 아니라 **React 트리 전체가 언마운트**됐다(2026-09-01 프로덕션, `/trade?symbol=AAPL`, `pageerror: [DecimalError] Invalid argument: undefined`, `root children: 0`). 34시간 시장 데이터 장애(§16.34) 동안 US 호가 프레임이 오지 않아 드러나지 않았고, 피드가 살아나자 바로 터졌다. 확정: **수량 필드는 전 계층 `volume` 하나**다(원장 컬럼 `book_level_volume`, `@moi/trading-core`의 `OrderBookLevel`, 엔진, 와이어가 이미 쓰는 낱말이며 `size`는 웹에만 있었다) — 웹 `BookLevel`을 `{price, volume}`으로 고친다. **프레임과 REST 스냅샷은 한 모양이다**: `MarketRuntime.#publishQuote`가 `quotePrice`(§16.33, REST와 같은 리더)로 구한 `price`와 `asOf`·`currency`·`bids`·`asks`를 실어 보내고, REST `#quote`도 같은 슬롯의 호가를 함께 돌려준다 — `#enrichPayload`가 portfolio에 대해 이미 말하는 규율(스냅샷과 패치가 두 모양이면 안 된다, §16.32)을 quote에도 적용한 것이다. 발명은 없다: 슬롯이 비면 `price: null`이고 호가는 **필드째로 생략**해(체결이 첫 호가보다 먼저 와도 클라이언트가 보고 있는 깊이를 지우지 않는다) `asOf`는 §16.17처럼 투영 시각이다. **체결도 프레임을 발행한다** — `quotePrice`가 마지막 체결을 우선하므로 체결이 표시가를 움직이는데 종전에는 호가 이벤트만 발행해서, 패널의 차트(`use-quote-ticks.ts` + `sparklineGeometry`, 2점부터 그린다)가 REST 첫 점 하나로 영구히 `Collecting…`에 머물렀다(보고된 "차트가 없다"의 실제 원인이며 크래시의 증상이 아니다). 웹은 캐스트 대신 경계에서 좁힌다: `lib/quote-frame.ts`의 `applyQuoteFrame(current, frame)`이 payload를 narrow 해 **화면의 quote 위에 병합**하고(호가 프레임이 가격을 재진술하지 않으면 스냅샷의 가격이 남는다), 좁힐 수 없는 프레임은 `null`로 **버려** 마지막 정상 quote가 남는다; 레벨은 `price`·`volume`이 모두 문자열인 것만 통과하고(숫자 가격은 거부 — API는 금액을 십진 문자열로 말한다), `depthPercent`·`maxVolume`은 `format-number.ts`가 렌더 경계에 대해 말하는 계약(십진수가 아니면 그대로 통과, 던지지 않는다)을 지켜 깨진 레벨을 깊이 0으로 그린다. e2e 하네스도 함께 고친다 — 이 결함을 e2e가 놓친 것은 우연이 아니라 하네스가 프로덕션의 대역이 아니었기 때문이다: `injectBook`이 호가를 엔진에 직접 넘기고 `publishQuote`를 **한 번도 부르지 않아** 브라우저에 `quote` 프레임이 간 적이 없고, 가짜 REST quote는 프로덕션이 서브하지 않는 모양(호가를 담고, 그것도 웹의 `size` 표기로)으로 답해 웹의 잘못된 믿음을 그대로 인코딩했다. `ErrorBoundary` 부재는 이 행에서 고치지 않았다(별건, 아래 후속). | `order-book.test.tsx`(포착한 프레임 렌더 · 깊이 스케일 · 누락/파싱불가 수량이 깊이 0 · `depthPercent`·`maxVolume` 전역성), `quote-frame.test.ts`(포착한 프레임 병합 · 프레임이 말한 가격/`asOf` 채택 · 숫자 가격 거부 · 좁힐 수 없는 payload는 `null` · 다른 종목의 가격을 물려받지 않음 · 레벨 narrowing), `quote-stream.test.tsx`(와이어 모양으로 교체한 픽스처 · 언마운트 없이 호가 렌더 · 프레임당 틱 누적으로 스파크라인 생성 · 호가만 있는 프레임은 틱을 만들지 않음), `market-runtime.test.ts` "states the price and the instant on the quote frame, not just the book"·"publishes a quote frame per trade even with no book yet"(원본 코드에서 RED), `production-runtime.integration.test.ts` quote 단언에 `currency`·`bids`·`asks` 추가, `quote-stream-panel.spec.ts`(e2e: 스트림 호가 렌더 + `pageerror` 0건 · 두 번째 가격에 차트 생성 · `size`로 잘못 적힌 레벨에도 생존) |
