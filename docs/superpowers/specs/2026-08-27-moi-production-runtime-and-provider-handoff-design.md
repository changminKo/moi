...
| 16.33 | §8.3 “5분 슬라이딩 창에서 3회 실패 시 자동 재시도 중단 + 수동 incident `RECOVERY_RETRY_EXHAUSTED`” · §8.2 재연결 | 34시간 시장 데이터 장애의 원인 두 건을 함께 고친다. **(a) 소진은 정지가 아니라 감속이다.** `ReconnectSupervisor`가 `#exhausted`를 영구 래치로 써서 `schedule()`이 첫 줄에서 되돌아왔고, 유일한 해제 경로가 관리자 라우트의 `resumeRetries()`였다 — 프로덕션 프로세스는 Toss 소켓 0개로 21시간을 조용히 보냈다. 소진 뒤에도 `schedule()`은 동작하되 지연을 `rearmDelayMs`(30 s부터 배증, 5 min 상한, 각 단계의 상위 절반에서 균등 추출 — full jitter가 아닌 half jitter인 이유는 하한이 0에 붙으면 상한을 둔 의미가 사라지기 때문이고, 지터가 필요한 이유는 KR·US가 같은 provider 사건으로 함께 소진되면 고정 지연이 두 시장의 재시도를 영구히 겹치게 하는데 둘이 공유하는 `OAuthTokenProvider`의 `TOKEN_MIN_REISSUE_INTERVAL_MS` 10 s 하한이 두 번째 호출자에게 `AUTH_THROTTLED`를 돌려주기 때문이다 — 한 시장의 실패가 다른 시장의 실패를 만들어낸다)로 바꾼다; `onExhausted`는 **진입 전이에서 한 번만** 울리고(재무장 실패마다 incident를 복제하지 않는다), `exhausted`는 경보 상태로 남는다. 재무장한 시도가 성공하면 `reset()`이 홀드를 풀고 `MarketRuntime`이 그 시장의 `RECOVERY_RETRY_EXHAUSTED` incident를 `resolveRetryExhausted`로 해제한다 — 그러지 않으면 피드가 돌아와도 시장은 `CANCEL_ONLY`에 남는다. `resume()`(운영자)은 대기 중인 재무장 타이머를 취소해 즉시 재시도한다. incident의 `source = 'MANUAL'`(§`MANUAL_CAUSES`)은 유지한다 — 사람이 봐야 할 사건이라는 표시일 뿐, 이제 복구의 **유일한** 경로는 아니다. 프로세스 단위 supervisor의 lease 재획득 루프는 소진 시 계속 던진다(`#acquireBundle`) — 동작 변경 없음. (중복 주의: 16.34가 `OPERATOR_ONLY_CAUSES` 기반의 일반 해소 정책으로 같은 홀드를 원장 행에서, 어느 프로세스가 연 행이든 해소한다. `resolveRetryExhausted`의 트리거는 인메모리 `supervisor.exhausted`라 재시작한 프로세스에서는 호출조차 되지 않는다 — 16.34가 머지되면 이 포트·어댑터·`#clearRetryHold()`의 호출을 제거하고 16.34 경로만 남긴다. `supervisor.resume()`과 `recovery.hold_cleared` 로그는 재시도 홀드가 supervisor 자신의 생명주기이므로 여기 남는다.) **(b) 교체된 소켓을 떼어낸다.** `handshake()`가 `this.socket`을 덮어쓰고 `this.closed = false`로 되돌리면서 이전 소켓을 닫지도, 그 `onclose`/`onmessage`/`onerror`를 떼지도 않았다. 죽은 소켓의 늦은 `onclose`가 `finish()`를 호출해 새 연결의 제어 대기자를 `{kind:'error', code:'transport-closed'}`로 깨웠고(`declare()`에는 `SUBSCRIPTION_REJECTED: declaration rejected: transport-closed`로 보였다), 버려진 소켓은 provider에 등록된 채 남아 다음 연결이 **중복 구독**이 되어 `"Bye"` 축출 → 재연결 → 다시 중복이라는 자기증폭 루프가 됐다(13시간 81회). 이제 `detachSocket()`이 핸들러를 `delete`하고 소켓을 닫은 뒤에야 새 소켓을 세우며, 소켓마다 세대 토큰을 부여해 핸들러가 자기 세대가 현재일 때만 동작한다(핸들러 참조를 이미 붙잡은 트랜스포트에 대한 두 번째 방어). 미읽은 이벤트 큐도 handshake에서 비운다 — 끝난 epoch의 `transportClosed`가 새 연결을 곧바로 degrade 시켰다. `close()`도 같은 detach를 쓴다. abort 리스너는 `close()`보다 먼저 settle 해 abort가 `AbortError`로 남게 한다. | `reconnect-supervisor.test.ts`(재무장, 상한을 지터 밴드 양끝에서 고정 — `random: () => 1`이면 `[30_000, 60_000, 120_000, 240_000, 300_000, 300_000]`, `random: () => 0`이면 `[15_000, 30_000, 60_000, 120_000, 150_000, 150_000]` — 같은 사건으로 소진된 두 scope의 지연 분리, 성공 시 해제), `market-runtime.test.ts` “keeps retrying after the hold and clears it without an operator”, `toss-websocket.reconnect.test.ts`(늦은 `onclose`·늦은 프레임·`close()` 후 재연결) |
