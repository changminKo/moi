...
| 16.15 | §6.1 RESTORING “열려 있는 주문·예약 로드” | `ProductionRuntime.#restoreOpenOrders()`가 `RECEIVED/OPEN/PARTIALLY_FILLED/TRIGGERED` 주문과 STOP/TAKE_PROFIT(단일은 DB status `OPEN`, OCO 다리는 `PENDING_TRIGGER`)을 `PaperEngine.restoreOrder()`로 주입한다(상태·`filledQuantity`·version 보존, 즉시 매칭 없음). 예약은 DB가 진실이므로 엔진에 별도 로드하지 않는다. | `production-runtime.integration.test.ts` RESTORING |
